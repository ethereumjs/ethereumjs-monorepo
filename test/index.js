...
tape('testing deletions cases', function(tester) {
  ...
  it('should delete from a branch->branch-branch', function(t) {
    async.parallel(
      [
        async.apply(trie.put.bind(trie), new Buffer([11, 11, 11]), Buffer.from('first')),
        async.apply(
          trie.put.bind(trie),
          new Buffer([12, 22, 22]),
  ...
  it('should delete from a extention->branch-extention', function(t) {
    trie.put(new Buffer([11, 11, 11]), Buffer.from('first'), function() {
      // create the top branch
      trie.put(new Buffer([12, 22, 22]), Buffer.from('create the first branch'), function() {
        // crete the middle branch
        trie.put(new Buffer([12, 33, 33]), Buffer.from('create the middle branch'), function() {
          trie.put(new Buffer([12, 33, 44]), Buffer.from('create the last branch'), function() {
  ...
  it('should delete from a extention->branch-branch', function(t) {
    trie.put(new Buffer([11, 11, 11]), Buffer.from('first'), function() {
      ...
      trie.put(new Buffer([12, 22, 22]), Buffer.from('create the first branch'), function() {
        // crete the middle branch
        trie.put(new Buffer([12, 33, 33]), Buffer.from('create the middle branch'), function() {
          trie.put(new Buffer([12, 34, 44]), Buffer.from('create the last branch'), function() {
            // delete the middle branch
            trie.del(new Buffer([11, 11, 11]), function() {
              trie.get(new Buffer([11, 11, 11]), function(err, val) {
                t.equal(null, val)
