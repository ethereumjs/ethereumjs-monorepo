...
module.exports = function (opts, cb) {
  ...
  function iterateVm (done) {
    ...
    async.series([
      prepareStep,
      addCodeInjectionListener,
      runStepHook,
      removeCodeInjectionListener,
      runOp
    ], function (err) {
      setImmediate(done.bind(null, err))
    })

    function prepareStep (cb) {
    ...
    function addCodeInjectionListener (cb) {
      self.on('injectNewCode', injectNewCode)
    ...
    function removeCodeInjectionListener (cb) {
      self.removeListener('injectNewCode', injectNewCode)

      cb()
    }

    function injectNewCode (content) {
      runState.code = new Buffer(content.code.replace(/^0x/, ''), 'hex')
