...
function StateManager (opts) {
  ...
  self.history.on('commit', cache.commit.bind(cache))
  self.history.on('revert', cache.revert.bind(cache))
}
...
proto.getAccount = function (address, cb) {
  ...
  if (account) {
    ...
    self._lookupAccount(address, function(err, account, exists){
      ...
      cache._cache = cache._cache.insert(address.toString('hex'), { val: account.serialize(), modified: false })
      cb(null, account, exists)
    })
  }
