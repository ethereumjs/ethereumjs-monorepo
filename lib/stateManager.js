...
proto.getAccount = function(address, cb){
  var self = this
  self.trie.get(new Buffer(address, 'hex'), function(err, raw){
...
proto.putAccount = function(address, account, cb){
  ...
  self.trie.put(new Buffer(address, 'hex'), account.serialize(), cb)
}

proto.setAccountBalance = function(address, account, balance, cb){
  var self = this
...
proto.setContractStorage = function(contract, key, value, cb){
  var self = this
  contract.setStorage(self.trie, key, value, cb)
}
...
proto.getBlockHashByNumber = function(number, cb) {
  this.blockchain.getBlockByNumber(number, function(err, block){
    if (err) return cb(err)
