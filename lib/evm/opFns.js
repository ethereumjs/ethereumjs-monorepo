...
module.exports = {
  ...
  EXTCODEHASH: async function (runState) {
    ...
    const stateManager = new PStateManager(runState.stateManager)
  ...
  CALL: async function (runState) {
    ...
    const state = new PStateManager(runState.stateManager)
    const empty = await state.accountIsEmpty(toAddress)
    if (empty) {
      if (!value.isZero()) {
