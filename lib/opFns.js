...
// EIP161: An account is considered empty when it has no code and zero nonce and zero balance.
function checkEmpty (runState, address, cb) {
  runState.getAccount(address, function (err, account) {
    if (err) {
      return cb(err)
    }

    cb(null, account.nonce === 0 && account.balance === 0 && account.code === '')
  })
}
...
module.exports = {
  ...
  CALL: function (gasLimit, toAddress, value, inOffset, inLength, outOffset, outLength, runState, done) {
    ...
    stateManager.exists(toAddress, function (err, exists) {
      ...
      checkEmpty(stateManager, toAddress, function (err, empty) {
        if (err) {
          done(err)
          return
        }
  ...
  SUICIDE: function (suicideToAddress, runState, cb) {
    ...
    stateManager.getAccount(suicideToAddress, function (err, toAccount) {
      ...
      checkEmpty(stateManager, suicideToAddress, function (error, empty) {
        if(error) {
          cb(error)
          return
        }
