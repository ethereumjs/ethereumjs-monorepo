...
module.exports = {
  ...
  CALL: function (gasLimit, toAddress, value, inOffset, inLength, outOffset, outLength, runState, done) {
    ...
    // add stipend
    if (!value.isZero()) {
      runState.gasLeft.isub(new BN(fees.callStipend.v))
...
function checkOutOfGas (runState, callOptions) {
  if (callOptions.gasLimit.gt(runState.gasLeft)) {
      const gasLeft = runState.gasLeft.toNumber();
      const newGas = gasLeft - Math.floor(gasLeft / 64)
      callOptions.gasLimit = new BN(newGas)
  }
  // if (callOptions.gasLimit.gt(runState.gasLeft)) {
  //   const newGas = runState.gasLimit.sub(runState.gasLimit.div(64))
