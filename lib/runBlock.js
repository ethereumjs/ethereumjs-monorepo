...
module.exports = function (opts, cb) {
      ...
      self.cache.flush(function () {
        if (validateStateRoot) {
          if (receiptTrie.root && receiptTrie.root.toString('hex') !== block.header.receiptTrie.toString('hex')) {
            ...
            err += 'invalid bloom '
          } else if (ethUtil.bufferToInt(block.header.gasUsed) !== Number(gasUsed)) {
            err += 'invalud gasUsed '
          } else if (self.trie.root.toString('hex') !== block.header.stateRoot.toString('hex')) {
  ...
  function payOmmersAndMiner () {
    ...
    minerAccount = self.cache.get(block.header.coinbase)
  ...
  function rewardOmmer (ommer) {
    ...
    var ommerAccount = self.cache.get(ommer.coinbase)
    ommerAccount.balance = reward.add(new BN(ommerAccount.balance))
    self.cache.put(ommer.coinbase, ommerAccount)
  }
