...
module.exports = {
  ...
  ADDRESS: function (eei) {
    return new BN(eei._runState.address)
  },
  BALANCE: function (address, eei, cb) {
    ...
    if (address.toString('hex') === eei._runState.address.toString('hex')) {
  ...
  ORIGIN: function (eei) {
    return new BN(eei._runState.origin)
  },
  CALLER: function (eei) {
    return new BN(eei._runState.caller)
  },
  CALLVALUE: function (eei) {
    return new BN(eei._runState.callValue)
  },
  ...
  CALLDATASIZE: function (eei) {
    if (eei._runState.callData.length === 1 && eei._runState.callData[0] === 0) {
      return new BN(0)
    } else {
      return new BN(eei._runState.callData.length)
    }
  },
  CALLDATACOPY: function (memOffset, dataOffset, dataLength, eei) {
    eei.memStore(memOffset, eei._runState.callData, dataOffset, dataLength)
    // sub the COPY fee
    eei.subGas(new BN(eei._runState._common.param('gasPrices', 'copy')).imul(dataLength.divCeil(new BN(32))))
  },
  CODESIZE: function (eei) {
    return new BN(eei._runState.code.length)
  },
  CODECOPY: function (memOffset, codeOffset, length, eei) {
    eei.memStore(memOffset, eei._runState.code, codeOffset, length)
    // sub the COPY fee
  ...
  RETURNDATASIZE: function (eei) {
    return new BN(eei._runState.lastReturned.length)
  },
  RETURNDATACOPY: function (memOffset, returnDataOffset, length, eei) {
    if ((returnDataOffset.add(length)).gtn(eei._runState.lastReturned.length)) {
      eei.trap(ERROR.OUT_OF_GAS)
    }

    eei.memStore(memOffset, utils.toBuffer(eei._runState.lastReturned), returnDataOffset, length, false)
    // sub the COPY fee
    eei.subGas(new BN(eei._runState._common.param('gasPrices', 'copy')).mul(length.divCeil(new BN(32))))
  },
  GASPRICE: function (eei) {
    return new BN(eei._runState.gasPrice)
  },
  // '0x40' range - block operations
  BLOCKHASH: function (number, eei, cb) {
    var blockchain = eei._runState.blockchain
    var diff = new BN(eei._runState.block.header.number).sub(number)
  ...
  COINBASE: function (eei) {
    return new BN(eei._runState.block.header.coinbase)
  },
  TIMESTAMP: function (eei) {
    return new BN(eei._runState.block.header.timestamp)
  },
  NUMBER: function (eei) {
    return new BN(eei._runState.block.header.number)
  },
  DIFFICULTY: function (eei) {
    return new BN(eei._runState.block.header.difficulty)
  },
  GASLIMIT: function (eei) {
    return new BN(eei._runState.block.header.gasLimit)
  },
  ...
  SLOAD: function (key, eei, cb) {
    ...
    stateManager.getContractStorage(eei._runState.address, key, function (err, value) {
      if (err) return cb(err)
  ...
  SSTORE: function (key, val, eei, cb) {
    ...
    var stateManager = eei._runState.stateManager
    var address = eei._runState.address
    key = key.toArrayLike(Buffer, 'be', 32)
  ...
  JUMP: function (dest, eei) {
    if (dest.gtn(eei._runState.code.length)) {
  ...
  GAS: function (eei) {
    return new BN(eei._runState.gasLeft)
  },
  JUMPDEST: function (eei) {},
  PUSH: function (eei) {
    const numToPush = eei._runState.opCode - 0x5f
    const loaded = new BN(eei._runState.code.slice(eei._runState.programCounter, eei._runState.programCounter + numToPush).toString('hex'), 16)
  ...
  CREATE2: function (value, offset, length, salt, eei, done) {
    ...
    var data = eei.memLoad(eei._runState, offset, length)
  ...
  CALL: function (gasLimit, toAddress, value, inOffset, inLength, outOffset, outLength, eei, done) {
    ...
    stateManager.accountIsEmpty(toAddress, function (err, empty) {
      ...
      if (!value.isZero()) {
        eei._runState.gasLeft.iaddn(eei._runState._common.param('gasPrices', 'callStipend'))
  ...
  CALLCODE: function (gas, toAddress, value, inOffset, inLength, outOffset, outLength, eei, done) {
    ...
    var data = eei.memLoad(eei._runState, inOffset, inLength)

    const options = {
      gasLimit: gas,
      value: value,
      data: data,
      to: eei._runState.address,
      static: eei._runState.static
    }
    ...
    eei.checkCallMemCost(eei._runState, options, localOpts)
    eei.checkOutOfGas(options)
  ...
  DELEGATECALL: function (gas, toAddress, inOffset, inLength, outOffset, outLength, eei, done) {
    var stateManager = eei._runState.stateManager
    var value = eei._runState.callValue
    toAddress = addressToBuffer(toAddress)
  ...
  SELFDESTRUCT: function (selfdestructToAddress, eei, cb) {
    ...
    var contractAddress = eei._runState.address
