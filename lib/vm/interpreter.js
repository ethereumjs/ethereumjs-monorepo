...
module.exports = class Interpreter {
  ...
  async executeMessage (message) {
    ...
    if (message.to) {
      ...
      ({ toAccount, createdAddress } = await this._createContract(account, message, salt))
    }
    ...
    const runCodeOpts = {
      ...
      block: this._block,
      depth: message.depth,
      // selfdestruct: selfdestruct, // TODO: where to store
      static: message.isStatic
      // storageReader: storageReader
    }

    // run Code through vm
    var codeRunner = isCompiled ? this._vm.runJIT : this._vm.runCode

    // Run code
    // results = await message.execute(runCodeOpts)
    let { err, results } = await new Promise((resolve, reject) => {
      codeRunner.call(this._vm, runCodeOpts, (err, results) => {
  ...
  async _parseRunResult (err, results, message, createdAddress, isCompiled) {
    if (createdAddress) {
      ...
      // if not enough gas
      if (totalGas.lte(message.gas) && (this._vm.allowUnlimitedContractSize || results.return.length <= 24576)) {
