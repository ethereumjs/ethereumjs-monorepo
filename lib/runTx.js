...
module.exports = function (opts, cb) {
  ...
  function populateCache (cb) {
    ...
    accounts.add(tx._from.toString('hex'))
  ...
  function runCall (cb) {
    ...
    // correct nonce
    var fromAccount = self.stateManager.cache.get(tx._from)
    var message
    ...
    self.stateManager.cache.put(tx._from, fromAccount)
    ...
    function parseResults (err, _results) {
      ...
      fromAccount = self.stateManager.cache.get(tx._from)
