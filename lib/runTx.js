...
module.exports = function(opts, cb) {
  ...
  function rewardSuicides(done) {
    ...
    var suicides = results.vm.suicides
    var keys = Object.keys(suicides)
    async.eachSeries(keys , function(key, cb2) {
      var s = suicides[key]
      var suicideAccount = self.cache.get(new Buffer(key, 'hex'))
      //load to account
      self.cache.getOrLoad(s, function(err, suicideToAccount) {
        //add to the balance
        suicideToAccount.balance = new BN(suicideToAccount.balance)
          .add(new BN(suicideAccount.balance))

        self.cache.put(s, suicideToAccount)
        cb2()
      })
    }, function(){
