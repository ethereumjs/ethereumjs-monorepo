...
module.exports = function (opts, cb) {
  ...
  if (!block) {
    block = new Block()
    block.header.gasLimit = '0xffffffff'
  }
  ...
  function runCall (cb) {
    ...
    if (new BN(fromAccount.balance).cmp(tx.getUpfrontCost()) === -1) {
      var message = "sender doesn't have enough funds to send tx. The upfront cost is: " + tx.getUpfrontCost().toString() + ' and the sender\s account only has: ' + new BN(fromAccount.balance).toString()
      cb(new Error(message))
      return
    } else if (!opts.skipNonce && new BN(fromAccount.nonce).cmp(new BN(tx.nonce)) !== 0) {
      var message = "the tx doesn't have the correct nonce. account has nonce of: " + new BN(fromAccount.nonce).toString() + ' tx has nonce of: ' + new BN(tx.nonce).toString()
      cb(new Error(message))
      return
    }
