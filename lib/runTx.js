...
module.exports = function (opts, cb) {
  ...
  function runCall (cb) {
    ...
    // correct nonce
    var fromAccount = self.stateManager.cache.get(opts.tx.from)
    ...
    function parseResults (err, _results) {
      ...
      fromAccount = self.stateManager.cache.get(opts.tx.from)
      ...
      self.stateManager.cache.put(opts.tx.from, fromAccount)

      var minerAccount = self.stateManager.cache.get(opts.block.header.coinbase)
      // add the amount spent on gas to the miner's account
      minerAccount.balance = new BN(minerAccount.balance)
        .add(results.amountSpent)

      // save the miner's account
      self.stateManager.cache.put(opts.block.header.coinbase, minerAccount)
