...
module.exports = function (opts, cb) {
  ...
  function runCall(done) {
    ...
    console.log('from: ' + opts.tx.from.toString('hex'));

    if (new BN(fromAccount.balance).cmp(opts.tx.getUpfrontCost()) === -1) {
      done('sender doesn\'t have enougth funds to send tx. The upfront cost is: ' + opts.tx.getUpfrontCost().toString() + ' and the sender\s account only has: ' + new BN(fromAccount.balance).toString())
    } else if (!opts.skipNonce && new BN(fromAccount.nonce).cmp(new BN(opts.tx.nonce)) !== 0) {
      done('the tx doesn\'t have the correct nonce. account has nonce of: ' + new BN(fromAccount.nonce).toString() + ' tx has nonce of: ' + new BN(opts.tx.nonce).toString())
    } else {
