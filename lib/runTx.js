...
module.exports = function (opts, cb) {
  ...
  function populateCache (cb) {
    ...
    accounts.add((tx.from || tx.getSenderAddress()).toString('hex'))
  ...
  function runCall (cb) {
    ...
    // correct nonce
    var fromAccount = self.stateManager.cache.get(tx.from || tx.getSenderAddress())
    var message
    ...
    self.stateManager.cache.put(tx.from || tx.getSenderAddress(), fromAccount)

    var options = {
      caller: tx.from || tx.getSenderAddress(),
      gasLimit: gasLimit,
    ...
    function parseResults (err, _results) {
      ...
      fromAccount = self.stateManager.cache.get(tx.from || tx.getSenderAddress())
