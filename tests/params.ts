...
tape('[Common]: Parameter access', function(t: tape.Test) {
  t.test('Basic usage', function(st: tape.Test) {
    ...
    st.equal(
      c.param('gasPrices', 'ecAdd', 'byzantium'),
      500,
      'Should return correct value when HF directly provided',
    )
  ...
  t.test('Error cases', function(st: tape.Test) {
    ...
    st.throws(
      ...
      /neither a hardfork set nor provided by param$/,
      'Should throw when no hardfork set or provided',
    )
    st.throws(
      function() {
        c.param('gasPrizes', 'ecAdd', 'byzantium')
      },
      /Topic gasPrizes not defined$/,
      'Should throw when called with non-existing topic',
    )
    st.throws(
      function() {
        c.param('gasPrices', 'notexistingvalue', 'byzantium')
      },
      /value for notexistingvalue not found$/,
      'Should throw when called with non-existing value',
    )
    ...
    st.throws(
      ...
      /supportedHardforks$/,
      'Should throw when calling param() with an unsupported hardfork',
    )
    st.throws(
      function() {
  ...
  t.test('Parameter updates', function(st: tape.Test) {
    ...
    st.throws(
      ...
      /value for ecAdd not found$/,
      'Should throw for a value set on a later HF',
    )

    st.equal(
      c.param('pow', 'minerReward', 'chainstart'),
      '5000000000000000000',
      'Should return correct value for chain start',
    )
    st.equal(
      c.param('pow', 'minerReward', 'byzantium'),
      '3000000000000000000',
      'Should reflect HF update changes',
    )
    st.equal(
      c.param('gasPrices', 'netSstoreNoopGas', 'constantinople'),
      200,
