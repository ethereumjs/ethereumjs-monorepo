...
tape('[Common]: Initialization / Chain params', function(t: tape.Test) {
  ...
  t.test('Should initialize with supportedHardforks provided', function(st: tape.Test) {
    ...
    st.equal(
      c._isSupportedHardfork('spuriousDragon'),
      false,
      'should return false for unsupported HF',
    )
  ...
  t.test('Should handle initialization errors', function(st: tape.Test) {
    st.throws(
      ...
      /not supported$/,
      'should throw an exception on non-existing chain',
    ...
    st.throws(
      ...
      /not supported$/,
      'should throw an exception on non-existing hardfork',
    ) // eslint-disable-line no-new
    st.throws(
      function() {
      ...
      /supportedHardforks$/,
      'should throw an exception on conflicting active/supported HF params',
    ) // eslint-disable-line no-new
  ...
  t.test('Should be able to access data for all chains provided', function(st: tape.Test) {
    ...
    st.equal(
      c.genesis().hash,
      '0xfa57319d09fd8a32faaf18d338c8a925a5a7975285bf29ecd024e083cba8abb1',
      'goerli',
    )
