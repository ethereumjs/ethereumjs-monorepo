...
import { EthereumJSErrorUnsetCode, bytesToHex, short } from '@ethereumjs/util'
...
async function executeBlocks(client: EthereumClient) {
  ...
  try {
    ...
    if ((blockRange[0][1] as string[]).length > 0 && blockRange.length === 2) {
      throw EthereumJSErrorUnsetCode('wrong input')
    }
  } catch (e: any) {
  ...
  if (execution === undefined) throw EthereumJSErrorUnsetCode('executeBlocks requires execution')
...
async function startBlock(client: EthereumClient) {
  ...
  if (height < startBlock) {
    throw EthereumJSErrorUnsetCode(`Cannot start chain higher than current height ${height}`)
  }
  try {
    ...
    throw EthereumJSErrorUnsetCode(`Error setting back chain in startBlock: ${err}`)
  }
}
...
async function startExecutionFrom(client: EthereumClient) {
  ...
  if (height < startExecutionFrom) {
    throw EthereumJSErrorUnsetCode(`Cannot start merkle chain higher than current height ${height}`)
  }
  ...
  if (genesisMeta.genesisState !== undefined || genesisMeta.genesisStateRoot !== undefined) {
    ...
    if (config.statefulVerkle) {
      if (genesisMeta.genesisState === undefined) {
        throw EthereumJSErrorUnsetCode('genesisState is required to compute stateRoot')
      }
