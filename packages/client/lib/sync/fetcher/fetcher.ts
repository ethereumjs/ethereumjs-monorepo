...
export abstract class Fetcher<JobTask, JobResult, StorageItem> extends Readable {
   ...
   */
  dequeue() {
    for (let f = this.out.peek(); f !== undefined && f.index <= this.processed; ) {
      this.processed++
  ...
  enqueueTask(task: JobTask, autoRestart = false) {
    if (this.errored !== undefined || (!this.running && !autoRestart)) {
      return
    }
  ...
  async fetch() {
    ...
    if (this.errored !== undefined) throw this.errored
  }
  ...
  private jobStr(job: Job<JobTask, JobResult, StorageItem>, withIndex = false) {
    ...
    if (this.isBlockFetcherJobTask(job.task)) {
      ...
      let partialResult = ''
      if (job.partialResult !== undefined) {
