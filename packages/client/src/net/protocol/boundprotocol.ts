import { EthereumJSErrorUnsetCode, Lock } from '@ethereumjs/util'
...
export class BoundProtocol {
      ...
      try {
        if (this.peer.pooled) {
          ...
          if (this.messageQueue.length >= 50) {
            const error = EthereumJSErrorUnsetCode('unexpected message queue growth for peer')
  ...
  private handle(incoming: Message) {
    ...
    try {
      ...
      error = EthereumJSErrorUnsetCode(`Could not decode message ${message.name}: ${e}`)
    }
  ...
  send(name: string, args?: any) {
    ...
    if (message) {
      ...
      throw EthereumJSErrorUnsetCode(`Unknown message: ${name}`)
    }
    return message
  }
  ...
  async request(name: string, args: any): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      resolver.timeout = setTimeout(() => {
        ...
        resolver.reject(EthereumJSErrorUnsetCode(`Request timed out after ${this.timeout}ms`))
      }, this.timeout)
    })
  }
}
