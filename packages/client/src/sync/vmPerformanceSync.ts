...
type PeerId = string
/**
 * Implements an ethereum vm performance sync synchronizer
 * The only purpose of this sync is to download the latest block and
 * then run this block. If any of the state requested is not in the DB
 * this is then requested over the devp2p/snap protocol
 * The purpose of this sync is to setup a performance test:
 * setup the local state DB for a certain block,
 * and then be able to run this block. This can be used to performance test
 * the VM + the client.
 * @memberof module:sync
 */
export class VMPerformanceSync extends Synchronizer {
  ...
  async syncWithPeer(peer?: Peer): Promise<boolean> {
    ...
    await peer!.eth?.getBlockHeaders({
      block: latest.parentHash,
      max: 1,
    })
