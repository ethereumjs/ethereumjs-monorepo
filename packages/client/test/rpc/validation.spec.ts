...
describe(`${prefix} should work without \`params\` when it's optional`, async () => {
...
describe(`${prefix} should return error without \`params\` when it's required`, async () => {
  const mockMethodName = 'mock'
  const server = startRPC({
...
describe(`${prefix} address`, () => {
  // valid
  // zero address
  assert.ok(validatorResult(validators.address(['0x0000000000000000000000000000000000000000'], 0)))
  // lowercase address
  assert.ok(validatorResult(validators.address(['0xa7d8d9ef8d8ce8992df33d8b8cf4aebabd5bd270'], 0)))
  // checksummed address
  assert.ok(validatorResult(validators.address(['0xa7d8d9ef8D8Ce8992Df33D8b8CF4Aebabd5bD270'], 0)))
  ...
  assert.notOk(validatorResult(validators.address(['0x00000000000000000000000000000000000000'], 0)))
...
describe(`${prefix} byteVectors`, () => {
  ...
  it('Bytes256', () => {
    // valid
    assert.ok(validatorResult(validators.bytes256([bytesToPrefixedHexString(randomBytes(256))], 0)))
...
describe(`${prefix} object`, () => {
  ...
  assert.notOk(validatorResult(validators.object({ bool: validators.bool })([{ bool: '0x0' }], 0)))
...
describe(`${prefix} array`, () => {
  ...
  assert.notOk(validatorResult(validators.array(validators.bool)([['0x123', '0x456', '0x789']], 0)))
...
describe(`${prefix} either`, () => {
  ...
  assert.notOk(validatorResult(validators.either(validators.hex, validators.blockHash)([true], 0)))
  assert.notOk(
    validatorResult(
      validators.either(
        validators.hex,
