...
describe(`${method}: call with earliest param`, async () => {
  const manager = createManager(createClient({ chain: createChain() }))
  const server = startRPC(manager.getMethods())
...
describe(`${method}: call with latest param`, async () => {
  const manager = createManager(createClient({ chain: createChain() }))
  const server = startRPC(manager.getMethods())
...
describe(`${method}: call with unimplemented pending param`, async () => {
  const manager = createManager(createClient({ chain: createChain() }))
  const server = startRPC(manager.getMethods())
...
describe(`${method}: call with non-string block number`, async () => {
  const manager = createManager(createClient({ chain: createChain() }))
  const server = startRPC(manager.getMethods())
...
describe(`${method}: call with transaction objects`, async () => {
  const manager = createManager(createClient({ chain: createChain() }))
  const server = startRPC(manager.getMethods())
