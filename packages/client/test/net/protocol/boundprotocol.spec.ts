...
describe("[BoundProtocol]", () => {
  ...
  const testMessage = {
    ...
    decode: (value: any) => Number.parseInt(value),
  };
  const testResponse = {
    name: "TestResponse",
    code: 0x02,
  ...
  it("should do handshake", async () => {
    ...
    td.when(protocol.handshake(td.matchers.isA(EventEmitter))).thenResolve({
      id: 1,
    });
  ...
  it("should perform send", () => {
    ...
    assert.deepEqual(
      bound.send("TestMessage", 3),
      testMessage,
      "message returned",
    );
    td.verify(sender.sendMessage(0x01, "3" as any));
    assert.throws(
      () => bound.send("UnknownMessage"),
      /Unknown message/,
      "unknown message",
    );
  });
