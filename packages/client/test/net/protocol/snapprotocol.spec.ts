...
tape('[SnapProtocol]', (t) => {
  ...
  t.test('AccountRange should encode/decode correctly',(t)=>{
    ...
    t.ok(secondAccount[2].toString("hex")==="3dc6d3cfdc6210b8591ea852961d880821298c7891dea399e02d87550af9d40e",'storageHash of the second account')
    t.ok(secondAccount[3].toString("hex")==="e68fe0bb7c4a483affd0f19cc2b989105242bd6b256c6de3afd738f8acd80c66",'codeHash of the second account')
    const payload = RLP.encode(p.encode(p.messages.filter((message) => message.name === 'AccountRange')[0], {reqId,accounts,proof}));
    t.ok(contractAccountRangeRLP===Buffer.from(payload).toString("hex"),"Re-encoded payload should match with original")
    t.end()
  })
  ...
  t.test('AccountRange should verify a real sample',async(t)=>{
    ...
    const {root:stateRoot} = p.decode(p.messages.filter((message) => message.name === 'GetAccountRange')[0], reqData);
   ...
   try{
    const accountRLP = await trie.verifyProof(stateRoot, accounts[accounts.length-1].hash, proof)
    if(accountRLP===null) {throw Error("Account should have existed in the verification trie");}
   }catch(e){
  ...
  t.test('StorageRanges should encode/decode correctly',(t)=>{
    ...
    const {reqId,slots,proof} = p.decode(p.messages.filter((message) => message.name === 'StorageRanges')[0], data);
    ...
    t.ok(hash.toString("hex")==="60264186ee63f748d340388f07b244d96d007fff5cbc397bbd69f8747c421f79",'Slot 3 key')
    t.ok(body.toString("hex")==="8462b66ae7",'Slot 3 value')

    const payload = RLP.encode(p.encode(p.messages.filter((message) => message.name === 'StorageRanges')[0], {reqId,slots,proof}));
    t.ok(storageRangesRLP===Buffer.from(payload).toString("hex"),"Re-encoded payload should match with original")
    t.end();
  })
  ...
  t.test('StorageRanges should verify a real sample',(t)=>{
    ...
    const {
          accounts,
        } = p.decode(p.messages.filter((message) => message.name === 'AccountRange')[0], accountsData);
    const lastAccount = accounts[accounts.length-1]

    /* eslint-disable @typescript-eslint/no-use-before-define */
    const data=RLP.decode(Buffer.from(storageRangesRLP,'hex')) as unknown
    const {proof} = p.decode(p.messages.filter((message) => message.name === 'StorageRanges')[0], data);
    t.ok(Buffer.from(keccak256(proof[0])).toString("hex") === (lastAccount.body as any)[2].toString('hex'),'Proof should link to the accounts storageRoot')
    t.end()
  })
...
const _getStorageRangesRLP=""
