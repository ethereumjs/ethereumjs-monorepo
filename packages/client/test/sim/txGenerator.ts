...
function getPadded(data: any, blobsLen: number) {
...
async function run(data: any) {
  const client = Client.http({ port: Number.parseInt(clientPort) })
  let done = false
  while (!done) {
    const num = Number.parseInt((await client.request('eth_blockNumber', [], 2.0)).result)
    if (num >= 1) {
      done = true
      break
    }
  ...
  const blobs = getBlobs(data).map((blob) => bytesToHex(blob))
  ...
  try {
    ...
    const start = Number.parseInt(res)
    for (let i = 0; i < 5; i++) {
      const res = (
  ...
  const expectedKzgs = blobTx.kzgCommitments![0]
  if (blobKzg !== blobTx.kzgCommitments![0]) {
    console.log(`Unexpected KZG commitment: expected ${expectedKzgs}, got ${blobKzg}`)
    return false
  } else {
    console.log(`Found expected KZG commitment: ${blobKzg}`)
  }
