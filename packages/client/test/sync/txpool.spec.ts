...
describe('[TxPool]', async () => {
  ...
  it('announcedTxHashes() -> add single tx / knownByPeer / getByHash()', async () => {
    ...
    for (const line of textLines) {
      if (pattern.test(line)) {
        feeMarketEip1559TransactionCountInPool = Number.parseInt(line.split(' ')[1])
      }
    }
    ...
    const peer = {
      eth: {
        ...
        getPooledTransactions: (res: any) => {
          assert.equal(res['hashes'].length, txRetrievalLimit, 'should limit to TX_RETRIEVAL_LIMIT')
          return [null, []]
        },
      },
    }
  ...
  it('announcedTxHashes() -> reject underpriced txn (same sender and nonce)', async () => {
    ...
    try {
      ...
      const poolObject = pool['handled'].get(bytesToUnprefixedHex(txA02Underpriced.hash()))
  ...
  it('announcedTxHashes() -> reject underpriced txn (same sender and nonce) in handleAnnouncedTxHashes', async () => {
    ...
    await pool.handleAnnouncedTxHashes([txA01.hash(), txA02Underpriced.hash()], peer, peerPool)
