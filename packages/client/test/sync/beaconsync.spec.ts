...
describe('[BeaconSynchronizer]', async () => {
  ...
  it('should extend and set with a valid head', async () => {
    ...
    const skeleton = new Skeleton({ chain, config, metaDB: new MemoryLevel() })
    const sync = new BeaconSynchronizer({ config, pool, chain, execution, skeleton })
    const head = blockFromBlockData({ header: { number: BigInt(15) } })
    ...
    await sync.open()
    const block = blockFromBlockData({ header: { number: BigInt(16), parentHash: head.hash() } })
    ...
    const gapBlock = blockFromBlockData({ header: { number: BigInt(18) } })
