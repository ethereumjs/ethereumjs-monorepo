...
describe('jwt', () => {
  it('jwt has `encode` method', () => {
    expect(jwt.encode).to.be.a('function')
  })

  it('jwt has `decode` method', () => {
...
describe('encode', () => {
  ...
  it('throw an error when the key is missing', () => {
    const fn = jwt.encode.bind(null, { foo: 'bar' })
    expect(fn).toThrowError(/Require key/)
  })

  it('throw an error when the specified algorithm is not supported', () => {
...
describe('decode', () => {
  ...
  it('throw an error when no token is provided', () => {
    //@ts-expect-error
    const fn = jwt.decode.bind(null, null, key)
    expect(fn).toThrowError(/No token supplied/)
  })

  it('throw an error when the token is not correctly formatted', () => {
  ...
  it('throw an error when the signature verification fails', () => {
    const fn = jwt.decode.bind(null, token, 'invalid_key')
    expect(fn).toThrowError(/Signature verification failed/)
  })

  it('throw an error when the token is not yet active (optional nbf claim)', () => {
  ...
  it('throw an error when the token has expired (optional exp claim)', () => {
  ...
  it('do not throw any error when verification is disabled', () => {
    const obj = { foo: 'bar' }
    const key = 'key'
    const token = jwt.encode(obj, key)
  ...
  it('decode token given algorithm', () => {
    const obj = { foo: 'bar' }
    const key = 'key'
  ...
  describe('RS256', () => {
    ...
    it('can add jwt header by options.header', () => {
    ...
    it('decode token given RS256 algorithm', () => {
      const token = jwt.encode(obj, pem, alg)
      const obj2 = jwt.decode(token, cert)
      expect(obj2).to.eql(obj)
    })

    it('throw an error when the key is invalid', () => {
      const token = jwt.encode(obj, pem, alg)
