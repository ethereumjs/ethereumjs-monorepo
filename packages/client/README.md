...
The client provides a quick way to get a local instance of the block chain up and running using the `--dev` command. This will start up a private PoA clique
...
This can also be paired with the `--unlock` command if you would like to specify the miner/prefunded account like below:
```shell
