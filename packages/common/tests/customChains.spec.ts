...
tape('[Common]: Custom chains', function (t: tape.Test) {
  ...
  t.test('customChains parameter: initialization', (st) => {
    ...
    st.equal(c.hardforkBlock().eqn(4), 'customChains, chain initialized with custom chain')
    st.deepEqual(
      c.genesisState(),
      {},
    ...
    st.equal(c.hardforkBlock().eqn(10), 'customChains, chain initialized with custom chain')
