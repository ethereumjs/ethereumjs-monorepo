...
tape('[Utils/Parse]', (t) => {
  t.test('should parse geth params file', async (t) => {
    const json = require('./data/geth-genesis/testnet.json')
  ...
  t.test('should throw with invalid Spurious Dragon blocks', async (t) => {
    t.plan(1)
    const json = require('./data/geth-genesis/invalid-spurious-dragon.json')
    try {
  ...
  t.test('should import poa network params correctly', async (t) => {
    t.plan(3)
    const json = require('./data/geth-genesis/poa.json')
  ...
  t.test(
    ...
    async (t) => {
      const json = require('./data/geth-genesis/post-merge.json')
  ...
  t.test('should successfully parse genesis file with no extraData', async (st) => {
    st.plan(2)
    const json = require('./data/geth-genesis/no-extra-data.json')
