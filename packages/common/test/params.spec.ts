...
describe("[Common]: Parameter instantiation / params option / Updates", () => {
  it("Param option", () => {
    ...
    msg =
      "Should update parameter on updateParams() and properly rebuild cache";
    assert.equal(c.param("bn254AddGas"), BigInt(250), msg);

    c.resetParams(params);
    msg =
      "Should reset all parameters on resetParams() and properly rebuild cache";
    assert.equal(c.param("bn254AddGas"), BigInt(250), msg);
    assert.throws(() => {
    ...
    msg =
      "Should not side-manipulate the original params file during updating internally";
...
describe("[Common]: Parameter access for param(), paramByHardfork()", () => {
  it("Basic usage", () => {
    ...
    assert.equal(
      c.paramByHardfork("bn254AddGas", "byzantium"),
      BigInt(500),
      msg,
    );
  ...
  it("Parameter updates", () => {
    ...
    assert.equal(
      c.paramByHardfork("netSstoreNoopGas", "constantinople"),
      BigInt(200),
      msg,
    );
    ...
    assert.equal(
      c.paramByHardfork("netSstoreNoopGas", "petersburg"),
      BigInt(0),
      msg,
    );
  });
  ...
  it("Access by block number, paramByBlock()", () => {
    ...
    assert.equal(
      c.paramByBlock("minerReward", 4370000),
      BigInt(3000000000000000000),
      msg,
    );
    ...
    assert.equal(
      c.paramByBlock("minerReward", 4369999),
      BigInt(5000000000000000000),
      msg,
    );
  });
  ...
  it("Access on copied Common instances", () => {
    ...
    msg =
      "Should correctly access param with param() on copied Common with hardfork changed";
  ...
  it("EIP param access, paramByEIP()", () => {
    ...
    const f = () => {
      c.paramByEIP("bls12381G1AddGas", unsupportedEip);
    };
