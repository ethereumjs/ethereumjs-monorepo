 ...
 * @param hardfork - The hardfork to use (defaults to Prague)
...
async function runPrecompile(
  ...
  console.log(`Precompile result: ${bytesToHex(res.returnValue)}`)
}

const main = async () => {
  // Example: Running BLS12_G1ADD precompile (address 0xb)
  ...
  const result = await runPrecompile('0xb', data)
}
