...
function create2address(sourceAddress: Address, codeHash: Uint8Array, salt: Uint8Array): Address {
  const rlpProcBytes = hexToBytes('0xff')
  const hashBytes = concatBytes(rlpProcBytes, sourceAddress.bytes, salt, codeHash)
...
*/
  ...
  */
  ...
  it('runCall() => use BLOBBASEFEE opcode from EIP 7516', async () => {
    ...
    const blobbasefeeOpcode = 0x4a
    assert.equal(
      evm.getActiveOpcodes().get(blobbasefeeOpcode)!.name,
      'BLOBBASEFEE',
