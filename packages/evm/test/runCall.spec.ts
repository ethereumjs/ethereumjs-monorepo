...
tape('Constantinople: EIP-1014 CREATE2 creates the right contract address', async (t) => {
  ...
  await evm.eei.putContractCode(contractAddress, Buffer.from(code, 'hex')) // setup the contract code
...
tape('Byzantium cannot access Constantinople opcodes', async (t) => {
  ...
  await evmByzantium.eei.putContractCode(contractAddress, Buffer.from(code, 'hex')) // setup the contract code
  await evmConstantinople.eei.putContractCode(contractAddress, Buffer.from(code, 'hex')) // setup the contract code
...
tape('Ensure that Istanbul sstoreCleanRefundEIP2200 gas is applied correctly', async (t) => {
  ...
  await evm.eei.putContractCode(address, Buffer.from(code, 'hex'))
  await evm.eei.putContractStorage(
    address,
    Buffer.alloc(32, 0),
