...
export function updateSstoreGasEIP2200(
  ...
  if (runState.interpreter.getGasLeft() <= common.param('gasPrices', 'sstoreSentryEIP2200Gas')) {
    trap(ERROR.OUT_OF_GAS)
  }

  // Noop
  if (equalsBytes(currentStorage, value)) {
    const sstoreNoopCost = common.param('gasPrices', 'sstoreNoopEIP2200Gas')
    return adjustSstoreGasEIP2929(runState, key, sstoreNoopCost, 'noop', common)
  }
  if (equalsBytes(originalStorage, currentStorage)) {
    // Create slot
    if (originalStorage.length === 0) {
      return common.param('gasPrices', 'sstoreInitEIP2200Gas')
    }
    // Delete slot
    if (value.length === 0) {
      runState.interpreter.refundGas(
        common.param('gasPrices', 'sstoreClearRefundEIP2200Gas'),
        'EIP-2200 -> sstoreClearRefundEIP2200'
      )
    }
    // Write existing slot
    return common.param('gasPrices', 'sstoreCleanEIP2200Gas')
  }
  if (originalStorage.length > 0) {
  ...
  if (equalsBytes(originalStorage, value)) {
    if (originalStorage.length === 0) {
      ...
      const sstoreInitRefund = common.param('gasPrices', 'sstoreInitRefundEIP2200Gas')
      runState.interpreter.refundGas(
      ...
      const sstoreCleanRefund = common.param('gasPrices', 'sstoreCleanRefundEIP2200Gas')
      runState.interpreter.refundGas(
  ...
  // Dirty update
  return common.param('gasPrices', 'sstoreDirtyEIP2200Gas')
}
