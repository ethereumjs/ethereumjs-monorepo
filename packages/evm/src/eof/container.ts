...
class EOFHeader {
  ...
  constructor(input: Uint8Array) {
    if (input.length > MAX_HEADER_SIZE) {
      throw EthereumJSErrorUnsetCode('err: container size more than maximum valid size')
    }
    ...
    if (input.length < 15) {
      throw EthereumJSErrorUnsetCode('err: container size less than minimum valid size')
    }
