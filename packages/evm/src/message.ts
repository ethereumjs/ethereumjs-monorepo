import { BIGINT_0, EthereumJSErrorUnsetCode, createZeroAddress } from '@ethereumjs/util'
...
export class Message {
  ...
  constructor(opts: MessageOpts) {
    ...
    if (this.value < 0) {
      throw EthereumJSErrorUnsetCode(`value field cannot be negative, received ${this.value}`)
    }
  }
  ...
  get codeAddress(): Address {
    ...
    if (!codeAddress) {
      throw EthereumJSErrorUnsetCode('Missing codeAddress')
    }
    return codeAddress
  }
}
