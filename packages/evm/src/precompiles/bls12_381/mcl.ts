...
function BLS12_381_ToG1Point(input: Uint8Array, mcl: any, verifyOrder = true): any {
  ...
  const pX = bytesToUnprefixedHex(input.subarray(16, BLS_G1_POINT_BYTE_LENGTH / 2))
  const pY = bytesToUnprefixedHex(input.subarray(80, BLS_G1_POINT_BYTE_LENGTH))

  const G1 = new mcl.G1()

  const fpX = new mcl.Fp()
  const fpY = new mcl.Fp()
...
function BLS12_381_ToG2Point(input: Uint8Array, mcl: any, verifyOrder = true): any {
  ...
  const pX1 = input.subarray(0, 64)
  const pX2 = input.subarray(64, BLS_G2_POINT_BYTE_LENGTH / 2)
  const pY1 = input.subarray(128, 192)
  const pY2 = input.subarray(192, BLS_G2_POINT_BYTE_LENGTH)

  const Fp2X = BLS12_381_ToFp2Point(pX1, pX2, mcl)
  const Fp2Y = BLS12_381_ToFp2Point(pY1, pY2, mcl)
...
function BLS12_381_ToFp2Point(fpXCoordinate: Uint8Array, fpYCoordinate: Uint8Array, mcl: any): any {
  ...
  fpX.setStr(bytesToUnprefixedHex(fpXCoordinate.subarray(16)), 16)
  fpY.setStr(bytesToUnprefixedHex(fpYCoordinate.subarray(16)), 16)
