...
export async function precompile13(opts: PrecompileInput): Promise<ExecResult> {
  ...
  if (opts._debug !== undefined) {
    opts._debug(
      `Run BLS12MAPFP2TOG2 (0x13) precompile data=${short(opts.data)} length=${
        opts.data.length
  ...
  if (opts.gasLimit < gasUsed) {
    if (opts._debug !== undefined) {
      opts._debug(`BLS12MAPFP2TOG2 (0x13) failed: OOG`)
    }
  ...
  for (const index in zeroByteCheck) {
    ...
    if (!(equalsBytes(slicedBuffer, zeroBytes16) === true)) {
      if (opts._debug !== undefined) {
        opts._debug(`BLS12MAPFP2TOG2 (0x13) failed: Point not on curve`)
      }
