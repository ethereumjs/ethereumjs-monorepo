...
import { BLS12_381_FromG2Point, BLS12_381_ToFrPoint, BLS12_381_ToG2Point } from './util/bls12_381'
...
export async function precompile0e(opts: PrecompileInput): Promise<ExecResult> {
  ...
  if (opts.gasLimit < gasUsed) {
    if (opts._debug !== undefined) {
      opts._debug(`BLS12G2MUL (0x0e) failed: OOG`)
    }
  ...
  if (inputData.length !== 288) {
    if (opts._debug !== undefined) {
      opts._debug(`BLS12G2MUL (0x0e) failed: Invalid input length length=${inputData.length}`)
    }
  ...
  if (opts._debug !== undefined) {
    opts._debug(`BLS12G2MUL (0x0e) return value=${bytesToHex(returnValue)}`)
  }
