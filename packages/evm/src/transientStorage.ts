...
export class TransientStorage implements TransientStorageInterface {
  ...
  public put(addr: Address, key: Uint8Array, value: Uint8Array) {
    if (key.length !== 32) {
      throw EthereumJSErrorUnsetCode('Transient storage key must be 32 bytes long')
    }

    if (value.length > 32) {
      throw EthereumJSErrorUnsetCode('Transient storage value cannot be longer than 32 bytes')
    }
   ...
   */
  public commit(): void {
    if (this._indices.length === 0) throw EthereumJSErrorUnsetCode('Nothing to commit')
  ...
  public revert() {
    ...
    if (typeof lastCheckpoint === 'undefined') throw EthereumJSErrorUnsetCode('Nothing to revert')
