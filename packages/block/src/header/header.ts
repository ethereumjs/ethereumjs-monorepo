...
export class BlockHeader {
  ...
  protected _validateDAOExtraData() {
    ...
    const daoExtraData = hexToBytes('0x64616f2d686172642d666f726b')
    const daoForceExtraDataRange = BigInt(9)
    const drift = this.number - DAOActivationBlock
    if (drift <= daoForceExtraDataRange && !equalsBytes(this.extraData, daoExtraData)) {
      const msg = this._errorMsg(
