...
import { EthereumJSErrorUnsetCode, bigIntToBytes, equalsBytes } from '@ethereumjs/util'
...
export function createBlockHeaderFromBytesArray(values: BlockHeaderBytes, opts: BlockOptions = {}) {
  ...
  if (header.common.isActivatedEIP(1559) && baseFeePerGas === undefined) {
    ...
    if (
      ...
      throw EthereumJSErrorUnsetCode('invalid header. baseFeePerGas should be provided')
    }
  }
  if (header.common.isActivatedEIP(4844)) {
    if (excessBlobGas === undefined) {
      throw EthereumJSErrorUnsetCode('invalid header. excessBlobGas should be provided')
    } else if (blobGasUsed === undefined) {
      throw EthereumJSErrorUnsetCode('invalid header. blobGasUsed should be provided')
    }
  }
  if (header.common.isActivatedEIP(4788) && parentBeaconBlockRoot === undefined) {
    throw EthereumJSErrorUnsetCode('invalid header. parentBeaconBlockRoot should be provided')
  }

  if (header.common.isActivatedEIP(7685) && requestsHash === undefined) {
    throw EthereumJSErrorUnsetCode('invalid header. requestsHash should be provided')
  }
  return header
}
...
export function createBlockHeaderFromRLP(
  ...
  if (!Array.isArray(values)) {
    throw EthereumJSErrorUnsetCode('Invalid serialized header input. Must be array')
  }
