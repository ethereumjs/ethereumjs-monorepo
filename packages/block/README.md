...
When [EIP-7843](https://eips.ethereum.org/EIPS/eip-7843) is active (`Hardfork.Amsterdam`, experimental), blocks carry a `slotNumber` header field. Set it explicitly when constructing blocks — `runBlock({ generate: true })` does not populate it automatically. The EVM `SLOTNUM` opcode (`0x4b`) reads this field — see [`@ethereumjs/evm` SLOTNUM](https://github.com/ethereumjs/ethereumjs-monorepo/tree/master/packages/evm#eip-7843-slotnum-opcode-amsterdam).
