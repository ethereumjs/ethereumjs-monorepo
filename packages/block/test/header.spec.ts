...
tape('[Block]: Header functions', function (t) {
  t.test('should create with default constructor', function (st) {
    function compareDefaultHeader(st: tape.Test, header: BlockHeader) {
      ...
      st.equals(header.difficulty, BigInt(0))
      st.equals(header.number, BigInt(0))
      st.equals(header.gasLimit, toType('0xffffffffffffff', TypeOutput.BigInt))
      st.equals(header.gasUsed, BigInt(0))
      st.equals(header.timestamp, BigInt(0))
  ...
  t.test('should return the baseFeePerGas from genesis if defined', function (st) {
    ...
    st.equals(header.baseFeePerGas!, BigInt(0x1000), 'correct baseFeePerGas')
    st.end()
  })
})
