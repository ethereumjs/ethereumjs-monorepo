...
tape('[Block]: Header functions', function (t) {
  t.test('should create with default constructor', function (st) {
    function compareDefaultHeader(st: tape.Test, header: BlockHeader) {
      ...
      st.ok(header.difficulty === BigInt(0))
      st.ok(header.number === BigInt(0))
      st.ok(header.gasLimit === toType('0xffffffffffffff', TypeOutput.BigInt))
      st.ok(header.gasUsed === BigInt(0))
      st.ok(header.timestamp === BigInt(0))
  ...
  t.test('should return the baseFeePerGas from genesis if defined', function (st) {
    ...
    st.ok(header.baseFeePerGas! === BigInt(1000), 'correct baseFeePerGas')
    st.end()
  })
})
