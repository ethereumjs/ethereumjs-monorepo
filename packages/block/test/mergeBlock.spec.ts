...
function validateMergeHeader(st: tape.Test, header: BlockHeader) {
  ...
  st.equals(header.difficulty, BigInt(0), 'difficulty')
  st.equals(header.number, BigInt(0), 'number')
  st.equals(header.gasLimit, toType('0xffffffffffffff', TypeOutput.BigInt), 'gasLimit')
  st.equals(header.gasUsed, BigInt(0), 'gasUsed')
  st.equals(header.timestamp, BigInt(0), 'timestamp')
  st.ok(header.extraData.length <= 32, 'extraData')
  st.equals(header.mixHash.length, 32, 'mixHash')
