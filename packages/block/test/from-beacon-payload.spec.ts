...
import { blockFromBeaconPayloadJson } from '../src/blockConstructor.js'
...
describe('[fromExecutionPayloadJson]: 4844 devnet 5', () => {
  ...
  it('reconstruct cancun block with blob txs', async () => {
    for (const payload of [payload87335, payload87475]) {
      try {
        const block = await blockFromBeaconPayloadJson(payload as BeaconPayloadJson, { common })
  ...
  it('should validate block hash', async () => {
    try {
      ...
      await blockFromBeaconPayloadJson(
        {
          ...payload87335,
  ...
  it('should validate excess blob gas', async () => {
    try {
      ...
      const block = await blockFromBeaconPayloadJson(
        {
          ...payload87475,
...
describe('[fromExecutionPayloadJson]: kaustinen', () => {
  ...
  it('reconstruct kaustinen block', async () => {
    ...
    const block = await blockFromBeaconPayloadJson(payloadKaustinen as BeaconPayloadJson, {
      common,
    })
