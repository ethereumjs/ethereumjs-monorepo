...
export class StatelessVerkleStateManager implements EVMStateManagerInterface {
  ...
  async clearContractStorage(address: Address): Promise<void> {
    ...
    const codeHashKey = getKey(stem, VerkleLeafType.CodeHash)
  ...
  async getAccount(address: Address): Promise<Account | undefined> {
    ...
    const stem = getStem(this.verkleCrypto, address, 0)
    const versionKey = getKey(stem, VerkleLeafType.Version)
    const balanceKey = getKey(stem, VerkleLeafType.Balance)
    const nonceKey = getKey(stem, VerkleLeafType.Nonce)
    const codeHashKey = getKey(stem, VerkleLeafType.CodeHash)
    const codeSizeKey = getKey(stem, VerkleLeafType.CodeSize)
