...
describe('StateManager -> General', () => {
  ...
  it(`should import proofs from getProof`, async () => {
    ...
    let stSlot10 = await partialStateManager.getStorage(address1, state1.keys[0])
    assert.ok(equalsBytes(stSlot10, state1.values[0]))

    let stSlot11 = await partialStateManager.getStorage(address1, state1.keys[1])
    assert.ok(equalsBytes(stSlot11, state1.values[1]))

    let stSlot12 = await partialStateManager.getStorage(address1, state1.keys[2])
    assert.ok(equalsBytes(stSlot12, new Uint8Array()))
    ...
    async function postVerify(sm: MerkleStateManager) {
      ...
      stSlot10 = await sm.getStorage(address1, state1.keys[0])
      assert.ok(equalsBytes(stSlot10, state1.values[0]))

      stSlot11 = await sm.getStorage(address1, state1.keys[1])
      assert.ok(equalsBytes(stSlot11, state1.values[1]))

      stSlot12 = await sm.getStorage(address1, state1.keys[2])
      assert.ok(equalsBytes(stSlot12, new Uint8Array()))
    }
