...
const { Fp, Fp2, Fp6, Fp4Square, Fp12 } = (0, tower_js_1.tower12)({
  ...
  Fp12cyclotomicExp(num, n) {
    ...
    for (let i = BLS_X_LEN - 1; i >= 0; i--) {
      ...
      if ((0, utils_js_1.bitGet)(n, i)) z = Fp12.mul(z, num)
    }
    return z
  },
...
const isogenyMapG2 = (0, hash_to_curve_js_1.isogenyMap)(
  Fp2,
  [
    // xNum
    [
...
const isogenyMapG1 = (0, hash_to_curve_js_1.isogenyMap)(
  Fp,
  [
    // xNum
    [
...
function signatureG2ToRawBytes(point) {
  ...
  return (0, utils_js_1.concatBytes)(
    setMask((0, utils_js_1.numberToBytesBE)(x1, len), { sort, compressed: true }),
    (0, utils_js_1.numberToBytesBE)(z2, len),
  )
}
/**
    ...
    toBytes: (c, point, isCompressed) => {
      ...
      if (isCompressed) {
        ...
        if (isZero) {
          ...
          const x = (0, utils_js_1.concatBytes)(
            new Uint8Array([0x40]),
          ...
          return (0, utils_js_1.concatBytes)(
            (0, utils_js_1.numberToBytesBE)(x, Fp.BYTES),
            (0, utils_js_1.numberToBytesBE)(y, Fp.BYTES),
          )
        }
      }
    },
    ShortSignature: {
      fromHex(hex) {
        const { infinity, sort, value } = parseMask(
          (0, utils_js_1.ensureBytes)('signatureHex', hex, 48),
        )
        const P = Fp.ORDER
        const compressedValue = (0, utils_js_1.bytesToNumberBE)(value)
        // Zero
    ...
    toBytes: (c, point, isCompressed) => {
      ...
      if (isCompressed) {
        if (isZero)
          return (0, utils_js_1.concatBytes)(
            COMPRESSED_ZERO,
            (0, utils_js_1.numberToBytesBE)(_0n, len),
          )
        const flag = Boolean(y.c1 === _0n ? (y.c0 * _2n) / P : (y.c1 * _2n) / P)
        return (0, utils_js_1.concatBytes)(
          setMask((0, utils_js_1.numberToBytesBE)(x.c1, len), { compressed: true, sort: flag }),
          (0, utils_js_1.numberToBytesBE)(x.c0, len),
        )
      } else {
        if (isZero)
        ...
        const { re: y0, im: y1 } = Fp2.reim(y)
        return (0, utils_js_1.concatBytes)(
          (0, utils_js_1.numberToBytesBE)(x1, len),
          (0, utils_js_1.numberToBytesBE)(x0, len),
          (0, utils_js_1.numberToBytesBE)(y1, len),
          (0, utils_js_1.numberToBytesBE)(y0, len),
        )
      }
    },
    Signature: {
      ...
      fromHex(hex) {
        const { infinity, sort, value } = parseMask(
          (0, utils_js_1.ensureBytes)('signatureHex', hex),
        )
        const P = Fp.ORDER
