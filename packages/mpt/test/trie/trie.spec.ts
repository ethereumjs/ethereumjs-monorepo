...
for (const { constructor, defaults, title } of [
  ...
  let rootDbKey: Uint8Array
  if (IS_SECURE_TRIE) {
    rootDbKey = keccak256(BASE_DB_KEY)
  } else {
    rootDbKey = BASE_DB_KEY
  }

  let expectedRoots: string
  if (IS_SECURE_TRIE) {
    expectedRoots = '0x8204723ce0fb452b130a282ecc727e07295c18cbd2c2eef33ba9eb9c7a9ded9b'
  } else {
    expectedRoots = '0x99650c730bbb99f6f58ce8b09bca2a8d90b36ac662e71bf81ec401ed23d199fb'
  }
  ...
  describe(`${title} (Persistence)`, () => {
    ...
    it('persist the root if the `useRootPersistence` option is `true`', async () => {
      ...
      assert.equal(await trie['_db'].get(rootDbKey), undefined)

      await trie.put(utf8ToBytes('foo'), utf8ToBytes('bar'))

      assert.equal(bytesToHex((await trie['_db'].get(rootDbKey))!), expectedRoots)
    })
    ...
    it('persist the root if the `root` option is given', async () => {
      ...
      assert.ok(equalsBytes((await trie['_db'].get(rootDbKey))!, KECCAK256_RLP))

      await trie.put(utf8ToBytes('foo'), utf8ToBytes('bar'))

      assert.isFalse(equalsBytes((await trie['_db'].get(rootDbKey))!, KECCAK256_RLP))
    })
    ...
    it('persists the root if the `db` option is not provided', async () => {
      ...
      assert.notEqual(await trie['_db'].get(rootDbKey), undefined)
    })
    ...
    it('persist and restore the root', async () => {
      ...
      assert.equal(bytesToHex((await copy['_db'].get(rootDbKey))!), expectedRoots)
      ...
      assert.equal(await empty['_db'].get(rootDbKey), undefined)
    })
  ...
  describe(`${title} (Check Root)`, async () => {
    ...
    it('should return false for all keys if trie is empty', async () => {
      ...
      assert.isFalse(
        await emptyTrie.checkRoot(emptyTrie['appliedKey'](rootDbKey)),
    ...
    it('Should throw on unrelated errors', async () => {
      ...
      try {
        await emptyTrie.checkRoot(rootDbKey)
...
describe('getValueMap', async () => {
  ...
  it('random key/value test', async () => {
    ...
    const keys = 1000
    const keyLen = 3 // Keys are of equal length
