import { EthereumJSErrorUnsetCode, equalsBytes } from '@ethereumjs/util'
...
async function unsetInternal(
  ...
  if (node instanceof ExtensionMPTNode || node instanceof LeafMPTNode) {
    ...
    if (shortForkLeft === -1 && shortForkRight === -1) {
      throw EthereumJSErrorUnsetCode('invalid range')
    }
...
async function verifyMPTWithMerkleProof(
  ...
  try {
    ...
    if (err.message === 'Missing node in DB') {
      throw EthereumJSErrorUnsetCode('Invalid proof provided')
    } else {
      throw err
    }
  }
...
export async function verifyMerkleRangeProof(
  ...
  if (keys.length !== values.length) {
    throw EthereumJSErrorUnsetCode('invalid keys length or values length')
  }

  // Make sure the keys are in order
  for (let i = 0; i < keys.length - 1; i++) {
    if (nibblesCompare(keys[i], keys[i + 1]) >= 0) {
      throw EthereumJSErrorUnsetCode('invalid keys order')
    }
  }
  // Make sure all values are present
  for (const value of values) {
    if (value.length === 0) {
      throw EthereumJSErrorUnsetCode('invalid values')
    }
  }
  ...
  if (proof === null && firstKey === null && lastKey === null) {
    ...
    if (!equalsBytes(rootHash, trie.root())) {
      throw EthereumJSErrorUnsetCode('invalid all elements proof: root mismatch')
    }
    return false
  }
  ...
  if (proof !== null && firstKey !== null && lastKey === null) {
    ...
    if (keys.length === 0) {
      ...
      if (value !== null || (await hasRightElement(trie, firstKey))) {
        throw EthereumJSErrorUnsetCode('invalid zero element proof: value mismatch')
      }
  ...
  if (keys.length === 1 && nibblesCompare(firstKey, lastKey) === 0) {
    ...
    if (value === null || !equalsBytes(value, values[0])) {
      throw EthereumJSErrorUnsetCode('invalid one element proof: value mismatch')
    }
  ...
  if (!equalsBytes(trie.root(), rootHash)) {
    throw EthereumJSErrorUnsetCode('invalid two edge elements proof: root mismatch')
  }
