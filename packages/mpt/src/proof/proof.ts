import { EthereumJSErrorUnsetCode, bytesToHex, concatBytes, equalsBytes } from '@ethereumjs/util'
  ...
  if (shouldVerifyRoot) {
    if (opStack[0] !== undefined && opStack[0] !== null) {
      if (!equalsBytes(trie.root(), opStack[0].key)) {
        throw EthereumJSErrorUnsetCode('The provided proof does not have the expected trie root')
      }
    }
  }
...
export async function verifyMPTWithMerkleProof(
  ...
  try {
    ...
    throw EthereumJSErrorUnsetCode('Invalid proof nodes given')
  }
  try {
    trie['DEBUG'] &&
