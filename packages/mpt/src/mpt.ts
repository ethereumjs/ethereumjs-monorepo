...
export class MerklePatriciaTrie {
  ...
  constructor(opts?: MPTOpts) {
    ...
    if (opts !== undefined) {
      ...
      if (opts?.valueEncoding !== undefined && opts.db === undefined) {
        throw EthereumJSErrorUnsetCode('`valueEncoding` can only be set if a `db` is provided')
      }
  ...
  async lookupNode(node: Uint8Array | Uint8Array[]): Promise<MPTNode> {
    ...
    if (value === null) {
      ...
      throw EthereumJSErrorUnsetCode('Missing node in DB')
    }
  ...
  protected async _updateNode(
    ...
    if (!lastNode) {
      throw EthereumJSErrorUnsetCode('Stack underflow')
    }
  ...
  protected async _deleteNode(k: Uint8Array, stack: MPTNode[]): Promise<void> {
    ...
    if (lastNode === undefined) throw EthereumJSErrorUnsetCode('missing last node')
  ...
  async saveStack(key: Nibbles, stack: MPTNode[], opStack: BatchDBOp[]): Promise<void> {
    ...
    while (stack.length) {
      ...
      if (node === undefined) {
        throw EthereumJSErrorUnsetCode('saveStack: missing node')
      }
    ...
    for (const op of ops) {
      if (op.type === 'put') {
        if (op.value === null || op.value === undefined) {
          throw EthereumJSErrorUnsetCode('Invalid batch db operation')
        }
