...
`@ethereumjs/genesis` provides genesis-state helpers and JSON genesis parsing. Amsterdam does not introduce new genesis formats in the `10.1.3` round; this package **bumps in version only**. Spin up Amsterdam test environments by setting `hardfork: Hardfork.Amsterdam` on your `Common` at runtime.
...
For Amsterdam execution and examples see [@ethereumjs/vm](https://github.com/ethereumjs/ethereumjs-monorepo/tree/master/packages/vm#amsterdam-hardfork-experimental).
