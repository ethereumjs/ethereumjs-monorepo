...
Master currently tracks [tests-glamsterdam-devnet@v7.0.0](https://github.com/ethereum/execution-specs/releases/tag/tests-glamsterdam-devnet%40v7.0.0) for the mixed Amsterdam tree. EIP-2780 / EIP-8037: intrinsic regular gas is **state-independent** (`txGas` + recipient/value/log extras + calldata + create access + `perAuthBaseGas`); the calldata floor is anchored on `TX_BASE` + those recipient extras ([execution-specs#3120](https://github.com/ethereum/execution-specs/pull/3120)). New-account state gas and 7702 `ACCOUNT_WRITE` / auth state are charged at top-frame access (`ACCOUNT_WRITE` counts toward receipt gas). Sender nonce is incremented before that prep layer (create-tx `NEW_ACCOUNT` OOG still bumps nonce). Create-tx `NEW_ACCOUNT` spill into `gas_left` is credited on REVERT so receipt gas can hit the calldata floor. Remaining EST gaps are leftover-pass work — do not treat the whole mix as green.
