...
const sdCommon = new Common({ chain: Chain.Mainnet, hardfork: Hardfork.SpuriousDragon })
sdCommon.setHardforkByBlockNumber(2772981)

// There was a consensus bug in the following tx;
// 0xe3b0fb0a45bc905d1f98baabaadd194901267d02de74cdad187b7feb8920d7b3
// This tx does not access any other accounts and creates a new contract, so we are safe to setup
// the account which creates this contract and just re-use the tx.
const txdata = {
...
tape('consensus bugs', async (t) => {
  t.test('validate out-of-gas does not refund any refunds', async (t) => {
    ...
    const tx = Transaction.fromTxData(txdata, { common: sdCommon })
