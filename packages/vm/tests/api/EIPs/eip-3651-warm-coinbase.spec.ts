...
tape('EIP 3651 tests', (t) => {
  t.test('invalid contract code transactions', async (st) => {
    ...
    const expectedDiff = new BN(common.param('gasPrices', 'coldaccountaccess')).subn(
      common.param('gasPrices', 'warmstorageread')
    )
    st.ok(result2.gasUsed.sub(result.gasUsed).eq(expectedDiff), 'gas difference is correct')
  })
})
