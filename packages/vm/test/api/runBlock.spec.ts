...
describe('runBlock() -> successful API parameter usage', async () => {
  async function simpleRun(vm: VM) {
    ...
    const genesis = blockFromRLPSerializedBlock(genesisRlp, { common })
  ...
  async function uncleRun(vm: VM) {
    ...
    const block1 = blockFromRLPSerializedBlock(block1Rlp, { common })
    await vm.runBlock({
      block: block1,
      root: (vm.stateManager as DefaultStateManager)['_trie'].root(),
      skipBlockValidation: true,
    ...
    const block2 = blockFromRLPSerializedBlock(block2Rlp, { common })
    await vm.runBlock({
      block: block2,
    ...
    const block3 = blockFromRLPSerializedBlock(block3Rlp, { common })
    await vm.runBlock({
      block: block3,
...
describe('runBlock() -> API parameter usage/data errors', async () => {
  ...
  it('should fail when block validation fails', async () => {
    ...
    const blockRlp = hexToBytes(testData.blocks[0].rlp as PrefixedHexString)
    const block = Object.create(blockFromRLPSerializedBlock(blockRlp, { common }))
...
describe('runBlock() -> runtime behavior', async () => {
  ...
  it('DAO fork behavior', async () => {
    ...
    const block = blockFromValuesArray(block1 as BlockBytes, { common })
    await setupPreConditions(vm.stateManager, testData)
