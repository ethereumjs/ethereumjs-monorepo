  ...
  it('runStateTest', async () => {
    try {
      ...
      if (testCases.length === 0) {
        assert.ok(`No ${options.forkConfigTestSuite} post state defined, skip test`)
        return
      }
      for (const testCase of testCases) {
        if (options.reps !== undefined && options.reps > 0) {
          ...
          for (let x = 0; x < options.reps; x++) {
            totalTimeSpent += await runTestCase(options, testCase)
          }
          assert.ok(`Average test run: ${(totalTimeSpent / options.reps).toLocaleString()} s`)
        } else {
          await runTestCase(options, testCase)
        }
      }
    } catch (e: any) {
      console.log(e)
