...
const selectedForksArgs = selectedForks.map((fork) => {
  const args = { ...defaultArgs }
  args.state = true
  args.fork = fork
  return args
})
describe(`VM SelectedForks Tests: ${selectedForks.toString()}`, async () => {
  for (const args of selectedForksArgs) {
    it('Homestead TangerineWhistle SpuriousDragon Petersburg Berlin London', async () => {
      await runTests(args) // eslint-disable-line @typescript-eslint/no-floating-promises
    })
  }
})
