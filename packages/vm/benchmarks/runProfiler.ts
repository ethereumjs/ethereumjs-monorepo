...
async function testJUMPDEST() {
  ...
  await runProfiler('JUMPDEST loop', codeStr, {
    gas: BigInt(1e8),
  })
}

async function testOpcode(stackPop: number, stackPush: number, opcode: number, name: string) {
  ...
  await runProfiler('Test opcode ' + name, codeStr, {
    gas: BigInt(30 * 1e6),
  })
}

async function testOpcodes() {
  ...
  for (const entry of tests) {
    ...
    await testOpcode(entry[1], entry[2], entry[3], entry[0])
  }
}
