...
// import { js_verify_update } from 'rust-verkle-wasm'

// TODO: VerkleTrie type
type VerkleTrie = any

/**
 * Options for constructing a {@link StateManager}.
 */
export interface VerkleStateManagerOpts {
  /**
   * Parameters of the chain {@link Common}
   */
  common?: Common
  /**
   * A {@link VerkleTrie} instance
   */
  verkleTrie?: VerkleTrie
}

/**
 * VerkleStateManager implementation for the VM.
 *
 ...
 */
export class VerkleStateManager extends BaseStateManager implements StateManager {
  _verkleTrie: VerkleTrie
  _storageVerkleTries: { [key: string]: VerkleTrie }

  /**
   * Instantiate the StateManager interface.
   */
  constructor(opts: VerkleStateManagerOpts = {}) {
    super(opts)

    this._verkleTrie = opts.verkleTrie
    this._storageVerkleTries = {}
    ...
    const getCb: getCb = async (address) => {
      const rlp = await this._verkleTrie.get(address.buf)
      return rlp ? Account.fromRlpSerializedAccount(rlp) : undefined
    }
    const putCb: putCb = async (keyBuf, accountRlp) => {
      const verkleTrie = this._verkleTrie
      await verkleTrie.put(keyBuf, accountRlp)
    }
    const deleteCb = async (keyBuf: Buffer) => {
      const verkleTrie = this._verkleTrie
      await verkleTrie.del(keyBuf)
    }
    this._cache = new Cache({ getCb, putCb, deleteCb })
  }

  /**
   * Copies the current instance of the `VerkleStateManager`
   * at the last fully committed point, i.e. as if all current
   * checkpoints were reverted.
   */
  copy(): StateManager {
    return new VerkleStateManager({
      verkleTrie: this._verkleTrie.copy(false),
      common: this._common,
    })
  }
  ...
  async putContractCode(address: Address, value: Buffer): Promise<void> {
    ...
    await this._verkleTrie.db.put(codeHash, value)
  ...
  async getContractCode(address: Address): Promise<Buffer> {
    ...
    const code = await this._verkleTrie.db.get(account.codeHash)
  ...
  async _lookupStorageTrie(address: Address): Promise<Trie> {
    // from state trie
    const account = await this.getAccount(address)
    const storageTrie = this._verkleTrie.copy(false)
  ...
  async _getStorageTrie(address: Address): Promise<Trie> {
    ...
    let storageTrie = this._storageVerkleTries[addressHex]
    if (!storageTrie) {
      // lookup from state
  ...
  async _modifyContractStorage(
    ...
    return new Promise(async (resolve) => {
      ...
      modifyTrie(storageTrie, async () => {
        ...
        const addressHex = address.buf.toString('hex')
        this._storageVerkleTries[addressHex] = storageTrie
  ...
  async getStateRoot(): Promise<Buffer> {
    ...
    const stateRoot = this._verkleTrie.root
    return stateRoot
  }
  ...
  async setStateRoot(stateRoot: Buffer): Promise<void> {
    ...
    if (!stateRoot.equals(this._verkleTrie.EMPTY_TRIE_ROOT)) {
      const hasRoot = await this._verkleTrie.checkRoot(stateRoot)
      if (!hasRoot) {
        throw new Error('State trie does not contain state root')
      }
    }

    this._verkleTrie.root = stateRoot
    this._cache.clear()
    this._storageVerkleTries = {}
  }
  ...
  async hasGenesisState(): Promise<boolean> {
    ...
    return await this._verkleTrie.checkRoot(toBuffer(root))
  }
  ...
  async accountExists(address: Address): Promise<boolean> {
    ...
    if (await this._verkleTrie.get(address.buf)) {
      return true
    }
    return false
  }
