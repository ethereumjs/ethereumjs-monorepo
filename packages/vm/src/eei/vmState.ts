...
export class VmState {
  ...
  async generateCanonicalGenesis(initState: any): Promise<void> {
    ...
    if (accounts.length > 0) {
      ...
      ;(this._stateManager as any)._trie = newTrie
    }
    for (const { hash, code } of byteCode) {
      await (this._stateManager as any)._trie.database().put(hash, code)
    }
    for (const { address, trie } of storageTries) {
      ;(this._stateManager as any)._storageTries[bytesToHex(address.bytes)] = trie
    }
    // await this._stateManager.flush()
