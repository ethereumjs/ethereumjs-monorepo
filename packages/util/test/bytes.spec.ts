...
describe('matchingBitsLength', () => {
  ...
  it('should return 0 when one of the arrays is empty', () => {
    const bits1 = new Uint8Array([1, 2, 3])
    const bits2 = new Uint8Array([])
    assert.equal(matchingBitsLength(bits1, bits2), 0)
  })

  it('should return 0 when arrays have no matching bits in the first byte', () => {
    // 0xff = 11111111, 0x7f = 01111111: first (most significant) bit mismatches.
    const bits1 = new Uint8Array([0xff])
    const bits2 = new Uint8Array([0x7f])
    assert.equal(matchingBitsLength(bits1, bits2), 0)
  })

  it('should return correct count for partially matching bits in the first byte', () => {
    // 0xff = 11111111, 0xf0 = 11110000:
    // The first four bits (1111) match, then the 5th bit mismatches.
    const bits1 = new Uint8Array([0xff])
    const bits2 = new Uint8Array([0xf0])
  ...
  it('should handle arrays with matching bits across multiple bytes', () => {
    ...
    const bits1 = new Uint8Array([0xaa, 0xff])
    const bits2 = new Uint8Array([0xaa, 0xf0])
  ...
  it('should handle arrays with same elements but different lengths', () => {
    ...
    const bits1 = new Uint8Array([0x12, 0x34, 0x56])
    const bits2 = new Uint8Array([0x12, 0x34, 0x56, 0x78])
  ...
  it('should handle arrays with matching bits at the start then mismatch mid-byte', () => {
    ...
    const bits1 = new Uint8Array([0x12, 0x34, 0x56])
    const bits2 = new Uint8Array([0x12, 0x34, 0x00])
    assert.equal(matchingBitsLength(bits1, bits2), 17)
  })

  it('should handle arrays with a large number of elements', () => {
    const length = 100000
    const arr1 = new Uint8Array(length)
    const arr2 = new Uint8Array(length)
    for (let i = 0; i < length; i++) {
      arr1[i] = i % 256
      arr2[i] = i % 256
    }
    // Each byte has 8 matching bits, so total matching bits = length * 8.
    assert.equal(matchingBitsLength(arr1, arr2), length * 8)
  })
})
