...
tape('encoding', function (t) {
  t.test('hexToCompact and compactToHex should encode hex data correctly', function (t) {
    ...
    for (let i = 0; i < tests.length; i++) {
      const test = tests[i]
      t.equal(JSON.stringify(hexToCompact(test.hex)), JSON.stringify(test.compact))
      t.equal(JSON.stringify(compactToHex(test.compact)), JSON.stringify(test.hex))
    }

    t.end()
  })

  t.test('keybytesToHex should encode data correctly', function (t) {
    const tests = [
    ...
    for (let i = 0; i < tests.length; i++) {
      const test = tests[i]
      t.equal(JSON.stringify(keybytesToHex(test.key)), JSON.stringify(test.hexOut))
    }
