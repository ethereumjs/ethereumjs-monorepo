  ...
  it("should return the right sender when using very high chain id / v values", () => {
    ...
    */
    ...
    const msgHash = hexToBytes(
      "0x8ae8cb685a7a9f29494b07b287c3f6a103b73fa178419d10d1184861a40f6afe",
    );

    const r = hexToBytes(
      "0xec212841e0b7aaffc3b3e33a08adf32fa07159e856ef23db85175a4f6d71dc0f",
    );
    const s = hexToBytes(
      "0x4b8e02b96b94064a5aa2f8d72bd0040616ba8e482a5dd96422e38c9a4611f8d5",
    );
...
describe("isValidSignature", () => {
  it("should fail on an invalid signature (shorter r))", () => {
    const r = hexToBytes(
      "0x99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1ab",
    );
    const s = hexToBytes(
      "0x129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca66",
    );
  ...
  it("should fail on an invalid signature (shorter s))", () => {
    ...
    const s = hexToBytes(
      "0x129ff05af364204442bdb53ab6f18a99ab48acc9326fa689f228040429e3ca",
    );
    assert.notOk(isValidSignature(BigInt(27), r, s));
  });
  it("should fail on an invalid signature (v = 21)", () => {
  ...
  it("should fail when on homestead and s > secp256k1n/2", () => {
    ...
    const s = bigIntToBytes(secp256k1NDiv2 + BigInt(1));
