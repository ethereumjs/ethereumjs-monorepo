...
describe("Address", () => {
  const zeroAddrS = "0x0000000000000000000000000000000000000000";
  ...
  it("should generate a zero address", () => {
    ...
    assert.deepEqual(addr.bytes, toBytes(zeroAddrS));
    assert.equal(addr.toString(), zeroAddrS);
  });

  it("should instantiate address from zero address string", () => {
    const addr = createAddressFromString(zeroAddrS);
    assert.deepEqual(addr.toString(), zeroAddrS);
  ...
  it("should generate address for created contract", () => {
    const from = createAddressFromString(
  ...
  it("should provide correct precompile check", () => {
    const precompile = createAddressFromString(
      "0x0000000000000000000000000000000000000009",
    );
    assert.isTrue(
      precompile.isPrecompileOrSystemAddress(),
      "should detect precompile address",
    );
    const nonPrecompile = createAddressFromString(
      "0x990ccf8a0de58091c028d6ff76bb235ee67c1c39",
    );
    assert.isFalse(
