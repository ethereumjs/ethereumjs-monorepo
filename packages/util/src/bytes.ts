...
export const hexToBytes = (hex: string) => {
  if (!isHexString(hex)) throw EthereumJSErrorWithoutCode('input string must be 0x prefixed')
  return nobleH2B(padToEven(stripHexPrefix(hex)))
}

export const unprefixedHexToBytes = (hex: string) => {
  if (isHexString(hex)) throw EthereumJSErrorWithoutCode('input string cannot be 0x prefixed')
