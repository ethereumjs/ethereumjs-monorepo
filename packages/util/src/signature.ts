 ...
 * @returns Signature
 */
export const toRPCSig = (v: bigint, r: Uint8Array, s: Uint8Array, chainId?: bigint): string => {
  const recovery = calculateSigRecovery(v, chainId)
  if (!isValidSigRecovery(recovery)) {
    throw new Error('Invalid signature v value')
  }
 ...
 * @returns Signature
 */
export const toCompactSig = (v: bigint, r: Uint8Array, s: Uint8Array, chainId?: bigint): string => {
  const recovery = calculateSigRecovery(v, chainId)
  if (!isValidSigRecovery(recovery)) {
    throw new Error('Invalid signature v value')
  }
 ...
 */
export const fromRPCSig = (sig: PrefixedHexString): ECDSASignature => {
