...
export const VERKLE_MAIN_STORAGE_OFFSET =
  BigInt(256) ** BigInt(VERKLE_CODE_CHUNK_SIZE);
...
export const chunkifyCode = (code: Uint8Array) => {
  ...
  if (code.length % VERKLE_CODE_CHUNK_SIZE !== 0) {
    const paddingLength =
      VERKLE_CODE_CHUNK_SIZE - (code.length % VERKLE_CODE_CHUNK_SIZE);
  ...
  for (let i = 1; i < Math.floor(code.length / 31); i++) {
    ...
    if (x >= 0 && slice[x] - 0x5f > 31 - x) {
      ...
      chunks.push(
        concatBytes(
          Uint8Array.from([pushDataOverflow]),
          code.slice(i * 31, (i + 1) * 31),
        ),
      );
    } else {
      // Put next chunk prepended with 0 (i.e. no overflow PUSHDATA bytes from previous chunk)
      chunks.push(
        concatBytes(new Uint8Array(1), code.slice(i * 31, (i + 1) * 31)),
      );
    }
  }
  return chunks;
