...
export class Address {
  ...
  constructor(bytes: Uint8Array) {
    if (bytes.length !== 20) {
      throw EthereumJSErrorUnsetCode('Invalid address length')
    }
    this.bytes = bytes
  }
...
export function createAddressFromBigInt(value: bigint): Address {
  ...
  if (bytes.length > 20) {
    throw EthereumJSErrorUnsetCode(`Invalid address, too long: ${bytes.length}`)
  }
...
export function createAddressFromString(str: string): Address {
  if (!isValidAddress(str)) {
    throw EthereumJSErrorUnsetCode(`Invalid address input=${str}`)
  }
...
export function createAddressFromPublicKey(pubKey: Uint8Array): Address {
  if (!(pubKey instanceof Uint8Array)) {
    throw EthereumJSErrorUnsetCode('Public key should be Uint8Array')
  }
...
export function createAddressFromPrivateKey(privateKey: Uint8Array): Address {
  if (!(privateKey instanceof Uint8Array)) {
    throw EthereumJSErrorUnsetCode('Private key should be Uint8Array')
  }
...
export function createContractAddress(from: Address, nonce: bigint): Address {
  if (typeof nonce !== 'bigint') {
    throw EthereumJSErrorUnsetCode('Expected nonce to be a bigint')
  }
...
export function createContractAddress2(
  ...
  if (!(salt instanceof Uint8Array)) {
    throw EthereumJSErrorUnsetCode('Expected salt to be a Uint8Array')
  }
  if (!(initCode instanceof Uint8Array)) {
    throw EthereumJSErrorUnsetCode('Expected initCode to be a Uint8Array')
  }
