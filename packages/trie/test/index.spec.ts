...
let cacheSize = 0
describe('simple save and retrieve', async () => {
  it('should not crash if given a non-existent root', async () => {
    ...
    const trie = new Trie({ rootHash: root })
...
describe('shall handle the case of node not found correctly', () => {
  it('should work', async () => {
    ...
    */
    ...
    let nodePath = await trie._getNodePath(bytesToNibbles(utf8ToBytes('aaa')), trie.debug)

    assert.ok(nodePath.node !== null, 'findPath should find a node')
    nodePath = await trie._getNodePath(bytesToNibbles(utf8ToBytes('aaa')), trie.debug)
    assert.deepEqual(nodePath.node.getValue(), utf8ToBytes('value3'))
    let fromDB = await trie.database().get(nodePath.node.hash())
    assert.notEqual(fromDB, null)
    await trie.database().del(nodePath.node.hash()) // delete the BranchNode -> value1 from the DB
    fromDB = await trie.database().get(nodePath.node.hash())
    assert.equal(fromDB, null)
    nodePath = await trie._getNodePath(bytesToNibbles(utf8ToBytes('aaa')), trie.debug)
    // assert.notDeepEqual(nodePath.node.getValue(), utf8ToBytes('value3'))
    // assert.equal(nodePath.node, null, 'findPath should not return a node now')
    assert.equal(
      nodePath.path[0].getType(),
      'ExtensionNode',
...
describe('setting back state root (deleteFromDB)', async () => {
  it('should work', async () => {
    ...
    assert.deepEqual(
      trieSetup.trie.root(),
      trieSetup.trie.EMPTY_TRIE_ROOT,
      'should return the correct empty state root'
    )
    const leafNode = new LeafNode({
      key: bytesToNibbles(k1),
      value: v1,
    })
    await trieSetup.trie.put(k1, v1)
    assert.deepEqual(await trieSetup.trie.get(k1), v1, 'should return v1 ')
    assert.equal(
      bytesToPrefixedHexString(trieSetup.trie.root()),
      bytesToPrefixedHexString(leafNode.hash()),
      'should return the correct state root'
    )
    const rootAfterK1 = trieSetup.trie.root()
...
describe('dummy hash', () => {
  it('should work', async () => {
    ...
    const expectedRoot = hashFunction(
      new LeafNode({ key: bytesToNibbles(k), value: v }).rlpEncode()
    )

    const trie = new Trie({ hashFunction, cacheSize })
    await trie.put(k, v)
...
describe('blake2b256 trie root', () => {
  it('should work', async () => {
    const trie = new Trie({ hashFunction: (msg) => blake2b(msg, 32), cacheSize })
...
//     const { node, path } = await trie._getNodePath(bytesToNibbles(utf8ToBytes('aaa')), trie.debug)
//     await trie.database().del(node.hash()) // delete the BranchNode -> value1 from the DB

//     nodePath = await trie._getNodePath(bytesToNibbles(utf8ToBytes('aaa')), trie.debug)

//     assert.equal(nodePath.node, null, 'findPath should not return a node now')
//     assert.equal(
//       nodePath.path.length,
//       1,
//       'findPath should find the first extension node which is still in the DB'
//     )
//   })
...
describe('setting back state root (deleteFromDB)', () => {
  it('should work', async () => {
    ...
    await trieSetup.trie.put(k1, v1)
    const rootAfterK1 = trieSetup.trie.root()
    assert.deepEqual(trieSetup.trie.root(), leaf1.hash(), 'should return the correct state root')
    assert.deepEqual(await trieSetup.trie.get(k1), v1, 'should return v1 ')
    await trieSetup.trie.put(k2, v2)
    assert.deepEqual(await trieSetup.trie.get(k1), v1, 'should return v1 ')
    assert.deepEqual(await trieSetup.trie.get(k2), v2, 'should return v2 ')
