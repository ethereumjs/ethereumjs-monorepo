...
import { CheckpointTrie, MapDB, ROOT_DB_KEY, Trie } from '../../src'

tape('SecureTrie', function (t) {
  const trie = new Trie({ useHashedKeys: true, db: new MapDB() })
  ...
  tape('secure tests', function (it) {
    let trie = new Trie({ useHashedKeys: true, db: new MapDB() })
    ...
    it.test('branchingTests', async function (t) {
      trie = new Trie({ useHashedKeys: true, db: new MapDB() })
    ...
    it.test('put fails if the key is the ROOT_DB_KEY', async function (st) {
      const trie = new Trie({ useHashedKeys: true, db: new MapDB(), persistRoot: true })
...
const trie = new CheckpointTrie({ useHashedKeys: true, db: new MapDB() })
const a = Buffer.from(
