...
describe('simple merkle range proofs generation and verification', () => {
  ...
  it('create a non-existent range proof and verify it', async () => {
    ...
    const endKey = prefixedHexStringToBytes('0x' + 'ff'.repeat(32))
  ...
  it('create a one element range proof and verify it', async () => {
    ...
    const tinyStartKey = prefixedHexStringToBytes('0x' + '00'.repeat(32))
  ...
  it('create all element range proof and verify it', async () => {
    ...
    assert.equal(
      await verify(
        ...
        0,
        entries.length - 1,
        prefixedHexStringToBytes('0x' + '00'.repeat(32)),
        prefixedHexStringToBytes('0x' + 'ff'.repeat(32))
      ),
      false
    )
  })
  ...
  it('should hasRightElement succeed', async () => {
    ...
    for (let { start, end, expect } of cases) {
      ...
      if (start === -1) {
        start = 0
        startKey = prefixedHexStringToBytes('0x' + '00'.repeat(32))
      } else {
      ...
      if (end === -1) {
        ...
        endKey = prefixedHexStringToBytes('0x' + 'ff'.repeat(32))
      } else {
