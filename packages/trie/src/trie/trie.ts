...
//     const { stack } = await this.getPath2(this.appliedKey(key), throwIfMissing)
...
//       const { remaining, stack } = await this.getPath(appliedKey)
//       let ops: BatchDBOp[] = []
...
//     await this._lock.acquire()
//     const appliedKey = this.appliedKey(key)
//     const { node, stack } = await this.getPath(appliedKey)
...
//    * @param throwIfMissing - if true, throws if any nodes are missing. Used for verifying proofs. (default: false)
//    */
//   async getPath(key: Uint8Array, throwIfMissing = false): Promise<Path> {
...
//    * @param throwIfMissing - if true, throws if any nodes are missing. Used for verifying proofs. (default: false)
//    */
//   async getPath2(key: Uint8Array, throwIfMissing = false): Promise<Path> {
...
//     } else {
//       // Dev note: this error message text is used for error checking in `checkRoot`, `verifyProof`, and `getPath`
//       throw new Error('Missing node in DB')
//     }
//     return foundNode
//   }
...
//     const { stack } = await this.getPath(this.appliedKey(key))
