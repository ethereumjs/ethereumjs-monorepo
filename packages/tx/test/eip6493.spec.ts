...
describe('ssz <> rlp converstion', async () => {
  ...
  for (const [txTypeName, txType, sszType, txData, txHash, txSender, common] of testCases) {
    it(`${txTypeName}`, () => {
      const origTx = txType.fromTxData(txData, { common })
      ...
      const origJson = origTx.toJSON()
      const origTxSszBytes = origTx.sszSerialize()
      const sszTx = txType.fromSszSerializedTx(origTxSszBytes, { common })
      const sszTxJson = sszTx.toJSON()
      assert.deepEqual(origJson, sszTxJson, 'the transaction jsons should match')

      // the serialized tx should be deserialized by top transaction container
      const stableTx = TransactionFactory.fromSszSerializedData(origTxSszBytes)
      const stableTxJson = stableTx.toJSON()
      assert.deepEqual(origJson, stableTxJson, 'the transaction jsons should match')
    })
  }

  it(`hashTree root of different transactions`, () => {
    const transactions = testCases.map(
      ([_txTypeName, txType, _sszType, txData, _txHash, _txSender, common]) => {
        const origTx = txType.fromTxData(txData, { common })
    ...
    assert.equal(
      ...
      '0x3edb1b359a9d44fe5b403c5764b52b20a5dfab6e4205d045a086a53aa16ec77c',
