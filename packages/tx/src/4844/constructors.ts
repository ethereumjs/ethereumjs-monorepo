...
const validateBlobTransactionNetworkWrapper = (
  ...
  if (blobVersionedHashes.length === 0) {
    throw EthereumJSErrorUnsetCode('Invalid transaction with empty blobs')
  }
  ...
  try {
    ...
    throw EthereumJSErrorUnsetCode(`KZG verification of blobs fail with error=${error}`)
  }
  if (!isValid) {
    throw EthereumJSErrorUnsetCode('KZG proof cannot be verified from blobs/commitments')
  }
...
export function createBlob4844TxFromSerializedNetworkWrapper(
  ...
  if (!opts || !opts.common) {
    throw EthereumJSErrorUnsetCode('common instance required to validate versioned hashes')
  }
