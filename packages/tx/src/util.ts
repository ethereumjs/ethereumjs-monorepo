...
/**
 * Retrieves the access list data from an input either in JSON or decoded RLP data
 * @param accessList The access list to be converted
 * @param skipCreateJSON Skip creating JSON if the format is already of decoded RLP data type (nested list of bytes)
 * @returns
 */
export function getAccessListData(
  accessList: AccessListBytes | AccessList,
  skipCreateJSON: boolean = false,
) {
  let accessListJSON
  ...
  if (isAccessList(accessList)) {
    ...
    if (!skipCreateJSON) {
      accessListJSON = getAccessListJSON(bufferAccessList)
    }
  }
...
/**
 * Retrieves the authorization list data from an input either in JSON or decoded RLP data
 * @param accessList The access list to be converted
 * @param skipCreateJSON Skip creating JSON if the format is already of decoded RLP data type (nested list of bytes)
 * @returns
 */
export function getAuthorizationListData(
  ...
  if (isAuthorizationList(authorizationList)) {
    ...
    if (!skipCreateJSON) {
      // build the JSON
      authorizationListJSON = getAuthorizationListJSON(bufferAuthorizationList)
    }
  }
