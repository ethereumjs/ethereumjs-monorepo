...
import { EthereumJSErrorUnsetCode, bytesToUtf8, utf8ToBytes } from '@ethereumjs/util'
...
export class ENR {
  ...
  static parseAndVerifyRecord(enr: string, common?: Common): PeerInfo {
    if (!enr.startsWith(this.RECORD_PREFIX))
      throw EthereumJSErrorUnsetCode(`String encoded ENR must start with '${this.RECORD_PREFIX}'`)
    ...
    if (!isVerified) throw EthereumJSErrorUnsetCode('Unable to verify ENR signature')
  ...
  static parseAndVerifyRoot(root: string, publicKey: string, common?: Common): string {
    if (!root.startsWith(this.ROOT_PREFIX))
      throw EthereumJSErrorUnsetCode(`ENR root entry must start with '${this.ROOT_PREFIX}'`)
    ...
    if (!rootValues.eRoot)
      throw EthereumJSErrorUnsetCode("Could not parse 'e' value from ENR root entry")
    if (!rootValues.lRoot)
      throw EthereumJSErrorUnsetCode("Could not parse 'l' value from ENR root entry")
    if (!rootValues.seq)
      throw EthereumJSErrorUnsetCode("Could not parse 'seq' value from ENR root entry")
    if (!rootValues.signature)
      throw EthereumJSErrorUnsetCode("Could not parse 'sig' value from ENR root entry")
    ...
    if (!isVerified) throw EthereumJSErrorUnsetCode('Unable to verify ENR root signature')
  ...
  static parseTree(tree: string): ENRTreeValues {
    if (!tree.startsWith(this.TREE_PREFIX))
      throw EthereumJSErrorUnsetCode(`ENR tree entry must start with '${this.TREE_PREFIX}'`)
    ...
    if (!treeValues.publicKey)
      throw EthereumJSErrorUnsetCode('Could not parse public key from ENR tree entry')
    if (!treeValues.domain)
      throw EthereumJSErrorUnsetCode('Could not parse domain from ENR tree entry')
  ...
  static parseBranch(branch: string): string[] {
    if (!branch.startsWith(this.BRANCH_PREFIX))
      throw EthereumJSErrorUnsetCode(`ENR branch entry must start with '${this.BRANCH_PREFIX}'`)
