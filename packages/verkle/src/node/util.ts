...
import { EthereumJSErrorUnsetCode, setLengthRight } from '@ethereumjs/util'
...
export const createCValues = (values: (Uint8Array | LeafVerkleNodeValue)[]) => {
  if (values.length !== 128)
    throw EthereumJSErrorUnsetCode(`got wrong number of values, expected 128, got ${values.length}`)
