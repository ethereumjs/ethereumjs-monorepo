...
tape('Clique: Initialization', (t) => {
  t.test('should initialize a clique blockchain', async (st) => {
    ...
    st.deepEquals(
      (blockchain as any).consensus.cliqueActiveSigners(),
  ...
  t.test('Clique Voting: Single signer, no votes cast', async (st) => {
    ...
    st.deepEqual((blockchain as any).consensus.cliqueActiveSigners(), [A.address])
    st.end()
  })
  ...
  t.test(
    ...
    async (st) => {
      ...
      st.deepEqual((blockchain as any).consensus.cliqueActiveSigners(), [A.address], 'fulfilled')
      st.end()
    }
  )
  ...
  t.test('Clique Voting: Three signers, two of them deciding to drop the third', async (st) => {
    ...
    st.deepEqual((blockchain as any).consensus.cliqueActiveSigners(), [A.address, B.address])
    st.end()
  })
  ...
  t.test(
    ...
    async (st) => {
      ...
      st.deepEqual((blockchain as any).consensus.cliqueActiveSigners(), [
        A.address,
        B.address,
        C.address,
        D.address,
  ...
  t.test('cliqueSignerInTurn() -> should work as expected', async (st) => {
    ...
    await addNextBlock(blockchain, blocks, B)
    st.notOk(await (blockchain as any).consensus.cliqueSignerInTurn(A.address))
    st.notOk(await (blockchain as any).consensus.cliqueSignerInTurn(B.address))
    st.ok(await (blockchain as any).consensus.cliqueSignerInTurn(C.address))
    // block 2: C, next signer: A
    await addNextBlock(blockchain, blocks, C)
    st.ok(await (blockchain as any).consensus.cliqueSignerInTurn(A.address))
    st.notOk(await (blockchain as any).consensus.cliqueSignerInTurn(B.address))
    st.notOk(await (blockchain as any).consensus.cliqueSignerInTurn(C.address))
    // block 3: A, next signer: B
    await addNextBlock(blockchain, blocks, A)
    st.notOk(await (blockchain as any).consensus.cliqueSignerInTurn(A.address))
    st.ok(await (blockchain as any).consensus.cliqueSignerInTurn(B.address))
    st.notOk(await (blockchain as any).consensus.cliqueSignerInTurn(C.address))
