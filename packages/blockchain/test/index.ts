...
test('blockchain test', (t) => {
  ...
  t.test('should add 10 blocks, one at a time', async (st) => {
    ...
    const addNextBlock = async (blockNumber: number) => {
      ...
      block.header.difficulty = toBuffer(
        block.header.canonicalDifficulty(blocks[blockNumber - 1])
      )
      block.header.gasLimit = toBuffer(8000000)
      block.header.timestamp = toBuffer(
        bufferToInt(blocks[blockNumber - 1].header.timestamp) + 1
      )
      await blockchain.putBlock(block)
      blocks.push(block)
      if (blocks.length < 10) {
        await addNextBlock(blockNumber + 1)
      } else {
  ...
  t.test('should get block by number', async (st) => {
    ...
    block.header.difficulty = toBuffer(
      block.header.canonicalDifficulty(blocks[0])
    )
    block.header.gasLimit = toBuffer(8000000)
    block.header.timestamp = toBuffer(
      bufferToInt(blocks[0].header.timestamp) + 1
    )
    blocks.push(block)
    await blockchain.putGenesis(genesisBlock)
    await blockchain.putBlock(block)
  ...
  t.test(
    'should get 5 consecutive blocks, starting from genesis hash',
    async (st) => {
  ...
  t.test(
    'should get 5 blocks, skipping 1 apart, starting from genesis hash',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
      ...
      st.ok(
        getBlocks![1].header.number.equals(blocks[2].header.number),
  ...
  t.test(
    'should get 4 blocks, skipping 2 apart, starting from genesis hash',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 10 consecutive blocks, starting from genesis hash',
    async (st) => {
  ...
  t.test(
    'should get 5 consecutive blocks, starting from block 0',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 5 blocks, skipping 1 apart, starting from block 1',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 5 blocks, skipping 2 apart, starting from block 0',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 15 consecutive blocks, starting from block 0',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(15)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 5 consecutive blocks, starting from block 1',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 5 consecutive blocks, starting from block 5',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 5 consecutive blocks, starting from block 5, reversed',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 6 consecutive blocks, starting from block 5, reversed',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(15)
      st.error(error, 'no error')
  ...
  t.test(
    'should get 6 blocks, starting from block 10, reversed, skipping 1 apart',
    async (st) => {
      const { blockchain, blocks, error } = await generateBlockchain(25)
      st.error(error, 'no error')
      ...
      st.equal(getBlocks!.length, 6)
      st.ok(
        getBlocks![1].header.number.equals(blocks[8].header.number),
        'should skip one block'
      )
      st.ok(
  ...
  t.test(
    'should not call iterator function in an empty blockchain',
    async (st) => {
      const blockchain = new Blockchain({
        validateBlocks: true,
        validatePow: false,
  ...
  t.test('should get meta.genesis', async (st) => {
    ...
    st.error(error, 'no error')
    st.ok(
      blockchain.meta.rawHead.equals(blocks[24].hash()),
      'should get meta.rawHead'
    )
    st.ok(
      blockchain.meta.genesis.equals(blocks[0].hash()),
  ...
  t.test('should add fork header and reset stale heads', async (st) => {
    ...
    forkHeader.timestamp = toBuffer(
      bufferToInt(blocks[14].header.timestamp) + 1
    )
    blockchain._heads['staletest'] = blockchain._headHeader
    await blockchain.putHeader(forkHeader)
    st.ok(
      blockchain._heads['staletest'].equals(blocks[14].hash()),
      'should update stale head'
    )
    st.ok(
      blockchain._headBlock.equals(blocks[14].hash()),
  ...
  t.test('should delete fork header', async (st) => {
    ...
    st.ok(
      blockchain._headHeader.equals(blocks[14].hash()),
  ...
  t.test('should delete blocks', async (st) => {
    ...
    st.ok(
      blockchain._headHeader.equals(blocks[5].hash()),
  ...
  t.test('should delete blocks and children', async (st) => {
    ...
    st.ok(
      blockchain._headHeader.equals(blocks[0].hash()),
  ...
  t.test('uncached db ops', async (st) => {
    ...
    st.ok(
      td.toBuffer().equals(genesis.header.difficulty),
  ...
  t.test('should save headers', async (st) => {
    ...
    st.ok(
      latestBlock.hash().equals(genesisBlock.hash()),
  ...
  t.test('immutable cached objects', async (st) => {
    ...
    block.header.difficulty = toBuffer(
      block.header.canonicalDifficulty(genesisBlock)
    )
    block.header.gasLimit = toBuffer(8000000)
    block.header.timestamp = toBuffer(
      bufferToInt(genesisBlock.header.timestamp) + 1
    )
    await blockchain.putBlock(block)
    const cachedHash = block.hash()

    block.header.extraData = Buffer.from([1])
    const getBlock = await blockchain.getBlock(1)
    st.ok(
      cachedHash.equals(getBlock.hash()),
  ...
  t.test('should get latest', async (st) => {
    ...
    const common = new Common({ chain: 'mainnet', hardfork: 'chainstart' })
    const headers = [
      new BlockHeader(undefined, { common }),
      new BlockHeader(undefined, { common }),
    ]
    const genesisBlock = new Block(undefined, { initWithGenesisHeader: true })
    genesisBlock.header.gasLimit = toBuffer(8000000)
    ...
    headers[0].difficulty = toBuffer(
      headers[0].canonicalDifficulty(genesisBlock)
    )
    headers[0].gasLimit = toBuffer(8000000)
    headers[0].timestamp = toBuffer(
      bufferToInt(genesisBlock.header.timestamp) + 1
    )
    ...
    const latestHeader = await blockchain.getLatestHeader()
    st.ok(
      latestHeader.hash().equals(headers[1].hash()),
    ...
    st.ok(
      latestHeader2.hash().equals(headers[1].hash()),
  ...
  t.test('mismatched chains', async (st) => {
    ...
    blocks[1].header.difficulty = toBuffer(
      blocks[1].header.canonicalDifficulty(blocks[0])
    )
    blocks[1].header.gasLimit = toBuffer(8000000)
    blocks[1].header.timestamp = toBuffer(
      bufferToInt(blocks[0].header.timestamp) + 1
    )

    blocks[2].header.number = toBuffer(2)
    blocks[2].header.parentHash = blocks[1].hash()
    blocks[2].header.difficulty = toBuffer(
      blocks[2].header.canonicalDifficulty(blocks[0])
    )
    blocks[2].header.gasLimit = toBuffer(8000000)
    blocks[2].header.timestamp = toBuffer(
      bufferToInt(blocks[0].header.timestamp) + 2
    )
