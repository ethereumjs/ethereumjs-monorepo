...
export class CliqueConsensus implements Consensus {
  ...
  async validateConsensus(block: Block): Promise<void> {
    if (!this.blockchain) {
      throw EthereumJSErrorUnsetCode('blockchain not provided')
    }

    const { header } = block
    const valid = cliqueVerifySignature(header, this.cliqueActiveSigners(header.number))
    if (!valid) {
      throw EthereumJSErrorUnsetCode('invalid PoA block signature (clique)')
    }
    if (this.cliqueCheckRecentlySigned(header)) {
      throw EthereumJSErrorUnsetCode('recently signed')
    }
  ...
  async validateDifficulty(header: BlockHeader): Promise<void> {
    ...
    if (header.difficulty !== CLIQUE_DIFF_INTURN && header.difficulty !== CLIQUE_DIFF_NOTURN) {
      ...
      throw EthereumJSErrorUnsetCode(`${msg} ${header.errorStr()}`)
    }
    ...
    throw EthereumJSErrorUnsetCode(`'invalid clique difficulty ${header.errorStr()}`)
  }
  ...
  cliqueActiveSigners(blockNum: bigint): Address[] {
    ...
    throw EthereumJSErrorUnsetCode(`Could not load signers for block ${blockNum}`)
  }
  ...
  async cliqueSignerInTurn(signer: Address, blockNum: bigint): Promise<boolean> {
    ...
    if (signerIndex === -1) {
      throw EthereumJSErrorUnsetCode('Signer not found')
    }
