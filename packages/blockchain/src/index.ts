...
export default class Blockchain implements BlockchainInterface {
    ...
    await this.runWithLock<void>(async () => {
      ...
      const rebuildInfo = async () => {
        ...
        if (isGenesis || block.transactions.length || block.uncleHeaders.length) {
          ...
          dbOps.push(
            DatabaseOperation.set(DatabaseOperationTarget.Body, bodyValue, {
              blockNumber,
              blockHash,
            })
          )
   ...
   * @hidden
   */
  private _saveHeadOps(): DatabaseOperation[] {
    return [
      DatabaseOperation.set(DatabaseOperationTarget.Heads, this._heads),
      DatabaseOperation.set(DatabaseOperationTarget.HeadHeader, this._headHeaderHash!),
      DatabaseOperation.set(DatabaseOperationTarget.HeadBlock, this._headBlockHash!),
    ]
  }
  ...
  private async _rebuildCanonical(header: BlockHeader, ops: DatabaseOperation[]) {
    ...
    const saveLookups = (blockHash: Buffer, blockNumber: BN) => {
      ops.push(
